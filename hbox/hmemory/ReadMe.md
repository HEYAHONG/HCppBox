...
|   `HFILEDESCRIPTOR_COMMON_TABLE_ATTRIBUTE`    |                文件描述符表的属性                 | 通常用于指定section信息，可用于在启用MMU时将文件描述符表存放至指定区域。 |
| `HFILEDESCRIPTOR_COMMON_TABLE_FLAG_ATTRIBUTE` | 文件描述符表标志的属性,用于存储文件描述符分配信息 | 通常用于指定section信息，可用于在启用MMU时将文件描述符表标志存放至指定区域。 |
