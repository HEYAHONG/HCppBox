...
size_t huint704_clz(const huint704_t *dst)
{
    if(dst!=NULL)
    {
        ...
        for(size_t i=index_start; i < HUINT704_BITS_COUNT; i++)
        {
...
void huint704_mul_with_stack(huint704_t *dst,const huint704_t *src1,const huint704_t *src2)
{
    ...
    huint704_mul(&state,dst,src1,src2);
}
...
void huint704_mul_with_external_state(huint704_state_t *state,huint704_t *dst,const huint704_t *src1,const huint704_t *src2)
{
    ...
    huint704_mul(&state->state[0],dst,src1,src2);
}
...
void huint704_div(huint704_t *state,huint704_t *state1,huint704_t *state2,huint704_t *dst,const huint704_t *src1,const huint704_t *src2)
{
    ...
    huint704_load_uint32(dst,0);
    if(huint704_compare(src2,dst)==0 )
    {
    ...
    huint704_copy(state,src1);
    size_t last_index=0;
    huint704_left_shift(state1,src2,(clz2-clz1)-last_index);
    for(size_t i=0; i<= clz2-clz1; i++)
    {
        huint704_right_shift_internal(state1,state1,i-last_index);
        last_index=i;
        if(huint704_compare(state,state1) >= 0)
        {
            ...
            {
                //求补码
                huint704_complement(state2,state1);
                //对补码进行加
                huint704_add(state,state,state2);
            }
...
void huint704_div_with_external_state(huint704_state_t * state,huint704_t *mod,huint704_t *dst,const huint704_t *src1,const huint704_t *src2)
{
    ...
    huint704_div(&state->state[0],&state->state[1],&state->state[2],dst,src1,src2);
    if(mod!=NULL)
    {
        huint704_copy(mod,&state->state[0]);
    }
}
...
void huint704_power_with_external_state(huint704_state_t * state,huint704_t *dst,const huint704_t *src1,const huint704_t *src2)
{
    ...
    huint704_power(&state->state[0],&state->state[1],&state->state[2],dst,src1,src2);
}

void huint704_root(huint704_t *state,huint704_t *state1,huint704_t *state2,huint704_t *state3,huint704_t *state4,huint704_t *dst,const huint704_t *src,size_t index)
{
    ...
    huint704_load_uint32(dst,0);
    huint704_load_uint64(state4,index);

    size_t dst_max_bit=(HUINT704_BITS_COUNT-huint704_clz(src)+index-1)/index;

    if(dst_max_bit*index > HUINT704_BITS_COUNT)
    {
        dst_max_bit--;
    }
    ...
    {
        huint704_bit_set(dst,dst_max_bit-i);
        huint704_power(state,state1,state2,state3,dst,state4);
        int compare_ret=huint704_compare(state3,src);
        if(compare_ret == 0)
        {
            break;
        }
        if(compare_ret > 0)
        {
            huint704_bit_clear(dst,dst_max_bit-i);
        }
    }

}

void huint704_root_with_stack(huint704_t *dst,const huint704_t *src,size_t index)
{
    huint704_t state[5];
    huint704_root(&state[0],&state[1],&state[2],&state[3],&state[4],dst,src,index);
}

void huint704_root_with_external_state(huint704_state_t * state,huint704_t *dst,const huint704_t *src,size_t index)
{
    if(state==NULL)
    {
        return;
    }
    huint704_root(&state->state[0],&state->state[1],&state->state[2],&state->state[3],&state->state[4],dst,src,index);
}
...
void huint704_power_mod_with_external_state(huint704_state_t * state,huint704_t *dst,const huint704_t *src1,const huint704_t *src2,const huint704_t *src3)
{
    ...
    huint704_power_mod(&state->state[0],&state->state[1],&state->state[2],&state->state[3],dst,src1,src2,src3);
}
...
void huint704_gcd(huint704_state_t * state,huint704_t *dst,const huint704_t *src1,const huint704_t *src2)
{
     ...
     * 寄存器6,7分别存储除数与被除数
     */
    if(huint704_compare(src1,src2) >=0)
    {
        huint704_copy(&state->state[7],src1);
        huint704_copy(&state->state[6],src2);
    }
    else
    {
        huint704_copy(&state->state[6],src1);
        huint704_copy(&state->state[7],src2);
    }
    /*
     * 寄存器4,5分别存储商与余数
     */
    huint704_load_uint32(&state->state[5],1);//初始时余数不为0
    /*
     * 寄存器3为0寄存器
     */
    huint704_load_uint32(&state->state[3],0);
    if(huint704_compare(&state->state[3],&state->state[6])==0)
    {
        /*
         * 当其中一个数为0时,返回较大的数
         */
        huint704_copy(dst,&state->state[7]);
        return;
    }

    do
    {
        huint704_div(&state->state[5],&state->state[0],&state->state[1],&state->state[4],&state->state[7],&state->state[6]);
        /*
         * 重新设置被除数与除数
         */
        huint704_copy(&state->state[7],&state->state[6]);
        huint704_copy(&state->state[6],&state->state[5]);
    }
    while(huint704_compare(&state->state[3],&state->state[5])!=0);
...
size_t huint704_dec_number_count(huint704_state_t * state,const huint704_t *src)
{
    ...
    while(!is_ok)
    {
         ...
         */
        huint704_load_uint32(&state->state[6],10);
        huint704_load_uint32(&state->state[7],dec_number_count);
        /*
         * 寄存器5存储10的指数的结果
         */
        huint704_power_with_external_state(state,&state->state[5],&state->state[6],&state->state[7]);

        /*
         * 比较大小
         */
        switch(huint704_compare(&state->state[5],src))
        {
        case 1:
        {
            is_ok=true;
...
size_t huint704_dec_number(huint704_state_t * state,const huint704_t *src,size_t index)
{
    size_t dec_number=0;
    if(state==NULL || src==NULL)
    {
     ...
     */
    huint704_load_uint32(&state->state[6],10);
    huint704_load_uint32(&state->state[7],index+1);
    /*
     * 寄存器5存储10的指数的结果
     */
    huint704_power_with_external_state(state,&state->state[5],&state->state[6],&state->state[7]);
     ...
     * 第一次除法
     */
    huint704_copy(&state->state[7],&state->state[5]);
    huint704_div_with_external_state(state,&state->state[4],&state->state[5],src,&state->state[7]);

    /*
     * 寄存器6存储10，寄存器7存储10的指数
     */
    huint704_load_uint32(&state->state[7],index);
    huint704_power_with_external_state(state,&state->state[5],&state->state[6],&state->state[7]);

    /*
     * 第二次除法
     */
    huint704_copy(&state->state[6],&state->state[4]);
    huint704_copy(&state->state[7],&state->state[5]);
    huint704_div_with_external_state(state,&state->state[4],&state->state[5],&state->state[6],&state->state[7]);

    uint32_t result=0;
    huint704_store_uint32(&state->state[5],&result);
    dec_number=result;
