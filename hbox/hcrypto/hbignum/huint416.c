...
size_t huint416_clz(const huint416_t *dst)
{
    if(dst!=NULL)
    {
        for(size_t i=0; i < HUINT416_BITS_COUNT; i++)
        {
...
void huint416_mul(huint416_t *state,huint416_t *dst,const huint416_t *src1,const huint416_t *src2)
{
    ...
    huint416_load_uint32(dst,0);
    size_t clz=huint416_clz(src2);
    size_t ctz=huint416_ctz(src2);
    size_t last_index=ctz;
    huint416_left_shift(state,src1,last_index);
    for(size_t i=ctz; i < (HUINT416_BITS_COUNT-clz); i++)
    {
        if(huint416_bit(src2,i))
        {
            //当前位是1, src1左移后累加至结果
            huint416_left_shift(state,state,i-last_index);
            last_index=i;
...
void huint416_div(huint416_t *state,huint416_t *state1,huint416_t *state2,huint416_t *dst,const huint416_t *src1,const huint416_t *src2)
{
    ...
    {
        huint416_right_shift(state1,state1,i-last_index);
        last_index=i;
        if(huint416_compare(state,state1) >= 0)
        {
            //被除数大于左移后的除数，直接相减并将相应位置1
            huint416_sub(state2,state,state1);
