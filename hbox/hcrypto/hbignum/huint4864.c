...
void huint4864_load_be(huint4864_t *num,const uint8_t *bytes,size_t bytes_count)
{
    ...
    {
        //丢弃高位
        size_t bytes_overflow=bytes_count-sizeof(huint4864_t);
...
static size_t huint4864_left_shift_internal(huint4864_t *dst,const huint4864_t *src,size_t bits)
{
    if(dst==NULL || src==NULL)
    {
        return 0;
...
void huint4864_left_shift(huint4864_t *dst,const huint4864_t *src,size_t bits)
{
    ...
    if(bits >= HUINT4864_BITS_COUNT)
    {
        //大于等于大数的位数，直接置0
        huint4864_load_uint32(dst,0);
        return;
    }

    huint4864_copy(dst,src);

    while(bits > 0)
    {
        bits-=huint4864_left_shift_internal(dst,dst,bits);
    }
}

static size_t huint4864_right_shift_internal(huint4864_t *dst,const huint4864_t *src,size_t bits)
{
    if(dst==NULL || src==NULL)
    {
        return 0;
...
void huint4864_right_shift(huint4864_t *dst,const huint4864_t *src,size_t bits)
{
    ...
    while(bits > 0)
    {
        bits-=huint4864_right_shift_internal(dst,dst,bits);
    }
}
...
size_t huint4864_clz(const huint4864_t *dst)
{
    if(dst!=NULL)
    {
        ...
        for(size_t i=index_start; i < HUINT4864_BITS_COUNT; i++)
        {
            if(huint4864_bit(dst,HUINT4864_BITS_COUNT-1-i))
            {
                return i;
            }
        }
...
void huint4864_sub(huint4864_t *dst,const huint4864_t *src1,const huint4864_t *src2)
{
    ...
    //求补码
    huint4864_complement(dst,src2);

    //对补码进行加
    huint4864_add(dst,dst,src1);
}
...
void huint4864_mul_with_stack(huint4864_t *dst,const huint4864_t *src1,const huint4864_t *src2)
{
    ...
    huint4864_mul(&state,dst,src1,src2);
}
...
void huint4864_mul_with_external_state(huint4864_state_t *state,huint4864_t *dst,const huint4864_t *src1,const huint4864_t *src2)
{
    ...
    huint4864_mul(&state->state[0],dst,src1,src2);
}
...
void huint4864_div(huint4864_t *state,huint4864_t *state1,huint4864_t *state2,huint4864_t *dst,const huint4864_t *src1,const huint4864_t *src2)
{
    ...
    huint4864_load_uint32(dst,0);
    if(huint4864_compare(src2,dst)==0 )
    {
        huint4864_load_uint32(state,0);
        //除0错误
        return;
    }

    if(huint4864_compare(src2,src1) > 0)
    {
        //除数大于被除数
        huint4864_copy(state,src1);
        huint4864_load_uint32(dst,0);
        return;
    }

    size_t clz1=huint4864_clz(src1);
    size_t clz2=huint4864_clz(src2);
    if(clz1 > clz2)
    {
        //除数大于被除数
        huint4864_copy(state,src1);
    ...
    huint4864_copy(state,src1);
    size_t last_index=0;
    huint4864_left_shift(state1,src2,(clz2-clz1)-last_index);
    for(size_t i=0; i<= clz2-clz1; i++)
    {
        huint4864_right_shift_internal(state1,state1,i-last_index);
        last_index=i;
        if(huint4864_compare(state,state1) >= 0)
        {
            //被除数大于左移后的除数，直接相减并将相应位置1
            {
                //求补码
                huint4864_complement(state2,state1);
                //对补码进行加
                huint4864_add(state,state,state2);
            }
            huint4864_bit_set(dst,(clz2-clz1)-i);
        }
    }
...
{
    huint4864_t state[3]= {0};
    huint4864_div(&state[0],&state[1],&state[2],dst,src1,src2);
    if(mod!=NULL)
    {
        huint4864_copy(mod,&state[0]);
    }
}
...
void huint4864_div_with_external_state(huint4864_state_t * state,huint4864_t *mod,huint4864_t *dst,const huint4864_t *src1,const huint4864_t *src2)
{
    ...
    huint4864_div(&state->state[0],&state->state[1],&state->state[2],dst,src1,src2);
    if(mod!=NULL)
    {
        huint4864_copy(mod,&state->state[0]);
    }
}
...
void huint4864_power(huint4864_t *state,huint4864_t *state1,huint4864_t *state2,huint4864_t *dst,const huint4864_t *src1,const huint4864_t *src2)
{
    ...
    huint4864_load_uint32(state,0);
    huint4864_load_uint32(dst,1);
    if(huint4864_compare(src1,state)==0 )
    {
        //底数为0
        huint4864_load_uint32(dst,0);
        return;
    }
    if(huint4864_compare(src2,state)==0 )
    {
        //任意数的0次方=1
        huint4864_load_uint32(dst,1);
        return;
    ...
    size_t clz2=huint4864_clz(src2);
    huint4864_copy(state,src1);
    for(size_t i=0; i<(HUINT4864_BITS_COUNT - clz2); i++)
    {
        if(huint4864_bit(src2,i))
        {
            //,结果应当乘上当前state的值(将幂函数的指数按照2进制进行拆分成乘法表达式)
            huint4864_mul(state1,state2,dst,state);
            huint4864_copy(dst,state2);
        }
        //计算state的平方（state的平方相当于state的指数乘2，正好对应src2下一个二进制位）
        huint4864_mul(state1,state2,state,state);
        huint4864_copy(state,state2);
    }
...
{
    huint4864_t state[3]= {0};
    huint4864_power(&state[0],&state[1],&state[2],dst,src1,src2);
}
...
void huint4864_power_with_external_state(huint4864_state_t * state,huint4864_t *dst,const huint4864_t *src1,const huint4864_t *src2)
{
    ...
    huint4864_power(&state->state[0],&state->state[1],&state->state[2],dst,src1,src2);
}
...
void huint4864_root(huint4864_t *state,huint4864_t *state1,huint4864_t *state2,huint4864_t *state3,huint4864_t *state4,huint4864_t *dst,const huint4864_t *src,size_t index)
{
    ...
    huint4864_load_uint32(dst,0);
    huint4864_load_uint64(state4,index);

    size_t dst_max_bit=(HUINT4864_BITS_COUNT-huint4864_clz(src)+index-1)/index;

    if(dst_max_bit*index > HUINT4864_BITS_COUNT)
    {
        dst_max_bit--;
    }
    ...
    {
        huint4864_bit_set(dst,dst_max_bit-i);
        huint4864_power(state,state1,state2,state3,dst,state4);
        int compare_ret=huint4864_compare(state3,src);
        if(compare_ret == 0)
        {
            break;
        }
        if(compare_ret > 0)
        {
            huint4864_bit_clear(dst,dst_max_bit-i);
        }
    }
...
{
    huint4864_t state[5];
    huint4864_root(&state[0],&state[1],&state[2],&state[3],&state[4],dst,src,index);
}
...
void huint4864_root_with_external_state(huint4864_state_t * state,huint4864_t *dst,const huint4864_t *src,size_t index)
{
    ...
    huint4864_root(&state->state[0],&state->state[1],&state->state[2],&state->state[3],&state->state[4],dst,src,index);
}
...
void huint4864_power_mod(huint4864_t *state,huint4864_t *state1,huint4864_t *state2,huint4864_t *state3,huint4864_t *dst,const huint4864_t *src1,const huint4864_t *src2,const huint4864_t *src3)
{
    ...
    for(size_t i=0; i<(HUINT4864_BITS_COUNT - clz2); i++)
    {
        if(huint4864_bit(src2,i))
        {
            ...
            huint4864_div(dst,state1,state2,state3,dst,src3);
        }
        //计算state的平方（state的平方相当于state的指数乘2，正好对应src2下一个二进制位）
        huint4864_mul(state1,state2,state,state);
        huint4864_copy(state,state2);
        ...
        huint4864_div(state,state1,state2,state3,state,src3);
    }
...
{
    huint4864_t state[4]= {0};
    huint4864_power_mod(&state[0],&state[1],&state[2],&state[3],dst,src1,src2,src3);
}
...
void huint4864_power_mod_with_external_state(huint4864_state_t * state,huint4864_t *dst,const huint4864_t *src1,const huint4864_t *src2,const huint4864_t *src3)
{
    ...
    huint4864_power_mod(&state->state[0],&state->state[1],&state->state[2],&state->state[3],dst,src1,src2,src3);
}
...
void huint4864_gcd(huint4864_state_t * state,huint4864_t *dst,const huint4864_t *src1,const huint4864_t *src2)
{
     ...
     * 寄存器6,7分别存储除数与被除数
     */
    if(huint4864_compare(src1,src2) >=0)
    {
        huint4864_copy(&state->state[7],src1);
        huint4864_copy(&state->state[6],src2);
    }
    else
    {
        huint4864_copy(&state->state[6],src1);
        huint4864_copy(&state->state[7],src2);
    }
    /*
     * 寄存器4,5分别存储商与余数
     */
    huint4864_load_uint32(&state->state[5],1);//初始时余数不为0
    /*
     * 寄存器3为0寄存器
     */
    huint4864_load_uint32(&state->state[3],0);
    if(huint4864_compare(&state->state[3],&state->state[6])==0)
    {
        /*
         * 当其中一个数为0时,返回较大的数
         */
        huint4864_copy(dst,&state->state[7]);
        return;
    }

    do
    {
        huint4864_div(&state->state[5],&state->state[0],&state->state[1],&state->state[4],&state->state[7],&state->state[6]);
        /*
         * 重新设置被除数与除数
         */
        huint4864_copy(&state->state[7],&state->state[6]);
        huint4864_copy(&state->state[6],&state->state[5]);
    }
    while(huint4864_compare(&state->state[3],&state->state[5])!=0);
...
size_t huint4864_dec_number_count(huint4864_state_t * state,const huint4864_t *src)
{
    ...
    while(!is_ok)
    {
         ...
         */
        huint4864_load_uint32(&state->state[6],10);
        huint4864_load_uint32(&state->state[7],dec_number_count);
        /*
         * 寄存器5存储10的指数的结果
         */
        huint4864_power_with_external_state(state,&state->state[5],&state->state[6],&state->state[7]);

        /*
         * 比较大小
         */
        switch(huint4864_compare(&state->state[5],src))
        {
        case 1:
        {
            is_ok=true;
...
size_t huint4864_dec_number(huint4864_state_t * state,const huint4864_t *src,size_t index)
{
     ...
     */
    huint4864_load_uint32(&state->state[6],10);
    huint4864_load_uint32(&state->state[7],index+1);
    /*
     * 寄存器5存储10的指数的结果
     */
    huint4864_power_with_external_state(state,&state->state[5],&state->state[6],&state->state[7]);
     ...
     * 第一次除法
     */
    huint4864_copy(&state->state[7],&state->state[5]);
    huint4864_div_with_external_state(state,&state->state[4],&state->state[5],src,&state->state[7]);

    /*
     * 寄存器6存储10，寄存器7存储10的指数
     */
    huint4864_load_uint32(&state->state[7],index);
    huint4864_power_with_external_state(state,&state->state[5],&state->state[6],&state->state[7]);

    /*
     * 第二次除法
     */
    huint4864_copy(&state->state[6],&state->state[4]);
    huint4864_copy(&state->state[7],&state->state[5]);
    huint4864_div_with_external_state(state,&state->state[4],&state->state[5],&state->state[6],&state->state[7]);

    uint32_t result=0;
    huint4864_store_uint32(&state->state[5],&result);
    dec_number=result;
