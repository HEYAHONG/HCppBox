...
size_t huint4864_clz(const huint4864_t *dst)
{
    if(dst!=NULL)
    {
        for(size_t i=0; i < HUINT4864_BITS_COUNT; i++)
        {
...
void huint4864_mul(huint4864_t *state,huint4864_t *dst,const huint4864_t *src1,const huint4864_t *src2)
{
    ...
    huint4864_load_uint32(dst,0);
    size_t clz=huint4864_clz(src2);
    size_t ctz=huint4864_ctz(src2);
    size_t last_index=ctz;
    huint4864_left_shift(state,src1,last_index);
    for(size_t i=ctz; i < (HUINT4864_BITS_COUNT-clz); i++)
    {
        if(huint4864_bit(src2,i))
        {
            //当前位是1, src1左移后累加至结果
            huint4864_left_shift(state,state,i-last_index);
            last_index=i;
...
void huint4864_div(huint4864_t *state,huint4864_t *state1,huint4864_t *state2,huint4864_t *dst,const huint4864_t *src1,const huint4864_t *src2)
{
    ...
    {
        huint4864_right_shift(state1,state1,i-last_index);
        last_index=i;
        if(huint4864_compare(state,state1) >= 0)
        {
            //被除数大于左移后的除数，直接相减并将相应位置1
            huint4864_sub(state2,state,state1);
