...
size_t huint6176_clz(const huint6176_t *dst)
{
    if(dst!=NULL)
    {
        for(size_t i=0; i < HUINT6176_BITS_COUNT; i++)
        {
...
void huint6176_mul(huint6176_t *state,huint6176_t *dst,const huint6176_t *src1,const huint6176_t *src2)
{
    ...
    huint6176_load_uint32(dst,0);
    size_t clz=huint6176_clz(src2);
    size_t ctz=huint6176_ctz(src2);
    size_t last_index=ctz;
    huint6176_left_shift(state,src1,last_index);
    for(size_t i=ctz; i < (HUINT6176_BITS_COUNT-clz); i++)
    {
        if(huint6176_bit(src2,i))
        {
            //当前位是1, src1左移后累加至结果
            huint6176_left_shift(state,state,i-last_index);
            last_index=i;
...
void huint6176_div(huint6176_t *state,huint6176_t *state1,huint6176_t *state2,huint6176_t *dst,const huint6176_t *src1,const huint6176_t *src2)
{
    ...
    {
        huint6176_right_shift(state1,state1,i-last_index);
        last_index=i;
        if(huint6176_compare(state,state1) >= 0)
        {
            //被除数大于左移后的除数，直接相减并将相应位置1
            huint6176_sub(state2,state,state1);
