...
size_t huint2432_clz(const huint2432_t *dst)
{
    if(dst!=NULL)
    {
        for(size_t i=0; i < HUINT2432_BITS_COUNT; i++)
        {
...
void huint2432_mul(huint2432_t *state,huint2432_t *dst,const huint2432_t *src1,const huint2432_t *src2)
{
    ...
    huint2432_load_uint32(dst,0);
    size_t clz=huint2432_clz(src2);
    size_t ctz=huint2432_ctz(src2);
    size_t last_index=ctz;
    huint2432_left_shift(state,src1,last_index);
    for(size_t i=ctz; i < (HUINT2432_BITS_COUNT-clz); i++)
    {
        if(huint2432_bit(src2,i))
        {
            //当前位是1, src1左移后累加至结果
            huint2432_left_shift(state,state,i-last_index);
            last_index=i;
...
void huint2432_div(huint2432_t *state,huint2432_t *state1,huint2432_t *state2,huint2432_t *dst,const huint2432_t *src1,const huint2432_t *src2)
{
    ...
    {
        huint2432_right_shift(state1,state1,i-last_index);
        last_index=i;
        if(huint2432_compare(state,state1) >= 0)
        {
            //被除数大于左移后的除数，直接相减并将相应位置1
            huint2432_sub(state2,state,state1);
