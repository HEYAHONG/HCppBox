...
size_t huint1024_clz(const huint1024_t *dst)
{
    if(dst!=NULL)
    {
        ...
        for(size_t i=index_start; i < HUINT1024_BITS_COUNT; i++)
        {
...
bool huint1024_t_is_zero(const huint1024_t * src)
{
    bool ret=false;
    if(src==NULL)
    {
...
bool huint1024_t_is_one(const huint1024_t * src)
{
    bool ret=false;
    if(src==NULL)
    {
...
void huint1024_mul_with_stack(huint1024_t *dst,const huint1024_t *src1,const huint1024_t *src2)
{
    huint1024_t state;

    huint1024_mul(&state,dst,src1,src2);
}

void huint1024_mul_with_external_state(huint1024_state_t *state,huint1024_t *dst,const huint1024_t *src1,const huint1024_t *src2)
{
    if(state==NULL)
    {
        return;
    ...
    huint1024_mul(&state->state[0],dst,src1,src2);
}
...
void huint1024_div(huint1024_t *state,huint1024_t *state1,huint1024_t *state2,huint1024_t *dst,const huint1024_t *src1,const huint1024_t *src2)
{
    ...
    huint1024_load_uint32(dst,0);
    if(huint1024_compare(src2,dst)==0 )
    {
    ...
    huint1024_copy(state,src1);
    size_t last_index=0;
    huint1024_left_shift(state1,src2,(clz2-clz1)-last_index);
    for(size_t i=0; i<= clz2-clz1; i++)
    {
        huint1024_right_shift_internal(state1,state1,i-last_index);
        last_index=i;
        if(huint1024_compare(state,state1) >= 0)
        {
            ...
            {
                //求补码
                huint1024_complement(state2,state1);
                //对补码进行加
                huint1024_add(state,state,state2);
            }
...
{
    huint1024_t state[3]= {0};
    huint1024_div(&state[0],&state[1],&state[2],dst,src1,src2);
    if(mod!=NULL)
    {
        huint1024_copy(mod,&state[0]);
    }
}

void huint1024_div_with_external_state(huint1024_state_t * state,huint1024_t *mod,huint1024_t *dst,const huint1024_t *src1,const huint1024_t *src2)
{
    if(state==NULL)
    {
        return;
    }
    huint1024_div(&state->state[0],&state->state[1],&state->state[2],dst,src1,src2);
    if(mod!=NULL)
    {
        huint1024_copy(mod,&state->state[0]);
    }
}
...
{
    huint1024_t state[3]= {0};
    huint1024_power(&state[0],&state[1],&state[2],dst,src1,src2);
}

void huint1024_power_with_external_state(huint1024_state_t * state,huint1024_t *dst,const huint1024_t *src1,const huint1024_t *src2)
{
    if(state==NULL)
    {
        return;
    }
    huint1024_power(&state->state[0],&state->state[1],&state->state[2],dst,src1,src2);
}

void huint1024_root(huint1024_t *state,huint1024_t *state1,huint1024_t *state2,huint1024_t *state3,huint1024_t *state4,huint1024_t *dst,const huint1024_t *src,size_t index)
{
    ...
    huint1024_load_uint32(dst,0);
    huint1024_load_uint64(state4,index);

    size_t dst_max_bit=(HUINT1024_BITS_COUNT-huint1024_clz(src)+index-1)/index;

    if(dst_max_bit*index > HUINT1024_BITS_COUNT)
    {
        dst_max_bit--;
    }
    ...
    {
        huint1024_bit_set(dst,dst_max_bit-i);
        huint1024_power(state,state1,state2,state3,dst,state4);
        int compare_ret=huint1024_compare(state3,src);
        if(compare_ret == 0)
        {
            break;
        }
        if(compare_ret > 0)
        {
            huint1024_bit_clear(dst,dst_max_bit-i);
        }
    }

}

void huint1024_root_with_stack(huint1024_t *dst,const huint1024_t *src,size_t index)
{
    huint1024_t state[5];
    huint1024_root(&state[0],&state[1],&state[2],&state[3],&state[4],dst,src,index);
}

void huint1024_root_with_external_state(huint1024_state_t * state,huint1024_t *dst,const huint1024_t *src,size_t index)
{
    if(state==NULL)
    {
        return;
    }
    huint1024_root(&state->state[0],&state->state[1],&state->state[2],&state->state[3],&state->state[4],dst,src,index);
}
...
{
    huint1024_t state[4]= {0};
    huint1024_power_mod(&state[0],&state[1],&state[2],&state[3],dst,src1,src2,src3);
}

void huint1024_power_mod_with_external_state(huint1024_state_t * state,huint1024_t *dst,const huint1024_t *src1,const huint1024_t *src2,const huint1024_t *src3)
{
    if(state==NULL)
    {
        return;
    }
    huint1024_power_mod(&state->state[0],&state->state[1],&state->state[2],&state->state[3],dst,src1,src2,src3);
}

void huint1024_gcd(huint1024_state_t * state,huint1024_t *dst,const huint1024_t *src1,const huint1024_t *src2)
{
    if(state==NULL || dst==NULL || src1==NULL || src2==NULL)
    {
        return;
     ...
     * 寄存器6,7分别存储除数与被除数
     */
    if(huint1024_compare(src1,src2) >=0)
    {
        huint1024_copy(&state->state[7],src1);
        huint1024_copy(&state->state[6],src2);
    }
    else
    {
        huint1024_copy(&state->state[6],src1);
        huint1024_copy(&state->state[7],src2);
    }
    /*
     * 寄存器4,5分别存储商与余数
     */
    huint1024_load_uint32(&state->state[5],1);//初始时余数不为0
    /*
     * 寄存器3为0寄存器
     */
    huint1024_load_uint32(&state->state[3],0);
    if(huint1024_compare(&state->state[3],&state->state[6])==0)
    {
        /*
         * 当其中一个数为0时,返回较大的数
         */
        huint1024_copy(dst,&state->state[7]);
        return;
    }

    do
    {
        huint1024_div(&state->state[5],&state->state[0],&state->state[1],&state->state[4],&state->state[7],&state->state[6]);
        /*
         * 重新设置被除数与除数
         */
        huint1024_copy(&state->state[7],&state->state[6]);
        huint1024_copy(&state->state[6],&state->state[5]);
    }
    while(huint1024_compare(&state->state[3],&state->state[5])!=0);
...
size_t huint1024_dec_number_count(huint1024_state_t * state,const huint1024_t *src)
{
    ...
    while(!is_ok)
    {
         ...
         */
        huint1024_load_uint32(&state->state[6],10);
        huint1024_load_uint32(&state->state[7],dec_number_count);
        /*
         * 寄存器5存储10的指数的结果
         */
        huint1024_power_with_external_state(state,&state->state[5],&state->state[6],&state->state[7]);

        /*
         * 比较大小
         */
        switch(huint1024_compare(&state->state[5],src))
        {
        case 1:
        {
            is_ok=true;
...
size_t huint1024_dec_number(huint1024_state_t * state,const huint1024_t *src,size_t index)
{
    size_t dec_number=0;
    if(state==NULL || src==NULL)
    {
     ...
     */
    huint1024_load_uint32(&state->state[6],10);
    huint1024_load_uint32(&state->state[7],index+1);
    /*
     * 寄存器5存储10的指数的结果
     */
    huint1024_power_with_external_state(state,&state->state[5],&state->state[6],&state->state[7]);
     ...
     * 第一次除法
     */
    huint1024_copy(&state->state[7],&state->state[5]);
    huint1024_div_with_external_state(state,&state->state[4],&state->state[5],src,&state->state[7]);

    /*
     * 寄存器6存储10，寄存器7存储10的指数
     */
    huint1024_load_uint32(&state->state[7],index);
    huint1024_power_with_external_state(state,&state->state[5],&state->state[6],&state->state[7]);

    /*
     * 第二次除法
     */
    huint1024_copy(&state->state[6],&state->state[4]);
    huint1024_copy(&state->state[7],&state->state[5]);
    huint1024_div_with_external_state(state,&state->state[4],&state->state[5],&state->state[6],&state->state[7]);

    uint32_t result=0;
    huint1024_store_uint32(&state->state[5],&result);
    dec_number=result;
