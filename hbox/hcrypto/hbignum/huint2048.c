...
void huint2048_mul(huint2048_t *state,huint2048_t *dst,const huint2048_t *src1,const huint2048_t *src2)
{
    ...
    huint2048_left_shift(state,src1,last_index);
    for(size_t i=ctz; i < (HUINT2048_BITS_COUNT-clz); i++)
    {
        if(huint2048_bit(src2,i))
        {
            //当前位是1, src1左移后累加至结果
            huint2048_left_shift(state,state,i-last_index);
            last_index=i;
...
void huint2048_div(huint2048_t *state,huint2048_t *state1,huint2048_t *state2,huint2048_t *dst,const huint2048_t *src1,const huint2048_t *src2)
{
    ...
    {
        huint2048_right_shift(state1,state1,i-last_index);
        last_index=i;
...
void huint2048_div_with_stack(huint2048_t *mod,huint2048_t *dst,const huint2048_t *src1,const huint2048_t *src2)
{
    huint2048_t state[3]= {0};
    huint2048_div(&state[0],&state[1],&state[2],dst,src1,src2);
    if(mod!=NULL)
    {
...
void huint2048_power_with_stack(huint2048_t *dst,const huint2048_t *src1,const huint2048_t *src2)
{
    huint2048_t state[3]= {0};
...
void huint2048_power_mod_with_stack(huint2048_t *dst,const huint2048_t *src1,const huint2048_t *src2,const huint2048_t *src3)
{
...
size_t huint2048_dec_number(huint2048_state_t * state,const huint2048_t *src,size_t index)
{
     ...
     */
    huint2048_load_uint32(&state->state[6],10);
    huint2048_load_uint32(&state->state[7],index+1);
    /*
     * 寄存器5存储10的指数的结果
     */
    huint2048_power_with_external_state(state,&state->state[5],&state->state[6],&state->state[7]);
    ...
    uint32_t result=0;
    huint2048_store_uint32(&state->state[5],&result);
    dec_number=result;
