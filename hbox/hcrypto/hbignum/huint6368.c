...
size_t huint6368_clz(const huint6368_t *dst)
{
    if(dst!=NULL)
    {
        for(size_t i=0; i < HUINT6368_BITS_COUNT; i++)
        {
...
void huint6368_mul(huint6368_t *state,huint6368_t *dst,const huint6368_t *src1,const huint6368_t *src2)
{
    ...
    huint6368_load_uint32(dst,0);
    size_t clz=huint6368_clz(src2);
    size_t ctz=huint6368_ctz(src2);
    size_t last_index=ctz;
    huint6368_left_shift(state,src1,last_index);
    for(size_t i=ctz; i < (HUINT6368_BITS_COUNT-clz); i++)
    {
        if(huint6368_bit(src2,i))
        {
            //当前位是1, src1左移后累加至结果
            huint6368_left_shift(state,state,i-last_index);
            last_index=i;
...
void huint6368_div(huint6368_t *state,huint6368_t *state1,huint6368_t *state2,huint6368_t *dst,const huint6368_t *src1,const huint6368_t *src2)
{
    ...
    {
        huint6368_right_shift(state1,state1,i-last_index);
        last_index=i;
        if(huint6368_compare(state,state1) >= 0)
        {
            //被除数大于左移后的除数，直接相减并将相应位置1
            huint6368_sub(state2,state,state1);
