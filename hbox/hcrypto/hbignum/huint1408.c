...
size_t huint1408_clz(const huint1408_t *dst)
{
    if(dst!=NULL)
    {
        for(size_t i=0; i < HUINT1408_BITS_COUNT; i++)
        {
...
void huint1408_mul(huint1408_t *state,huint1408_t *dst,const huint1408_t *src1,const huint1408_t *src2)
{
    ...
    huint1408_load_uint32(dst,0);
    size_t clz=huint1408_clz(src2);
    size_t ctz=huint1408_ctz(src2);
    size_t last_index=ctz;
    huint1408_left_shift(state,src1,last_index);
    for(size_t i=ctz; i < (HUINT1408_BITS_COUNT-clz); i++)
    {
        if(huint1408_bit(src2,i))
        {
            //当前位是1, src1左移后累加至结果
            huint1408_left_shift(state,state,i-last_index);
            last_index=i;
...
void huint1408_div(huint1408_t *state,huint1408_t *state1,huint1408_t *state2,huint1408_t *dst,const huint1408_t *src1,const huint1408_t *src2)
{
    ...
    {
        huint1408_right_shift(state1,state1,i-last_index);
        last_index=i;
        if(huint1408_compare(state,state1) >= 0)
        {
            //被除数大于左移后的除数，直接相减并将相应位置1
            huint1408_sub(state2,state,state1);
