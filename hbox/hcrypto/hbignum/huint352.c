...
size_t huint352_clz(const huint352_t *dst)
{
    if(dst!=NULL)
    {
        for(size_t i=0; i < HUINT352_BITS_COUNT; i++)
        {
...
void huint352_mul(huint352_t *state,huint352_t *dst,const huint352_t *src1,const huint352_t *src2)
{
    ...
    huint352_load_uint32(dst,0);
    size_t clz=huint352_clz(src2);
    size_t ctz=huint352_ctz(src2);
    size_t last_index=ctz;
    huint352_left_shift(state,src1,last_index);
    for(size_t i=ctz; i < (HUINT352_BITS_COUNT-clz); i++)
    {
        if(huint352_bit(src2,i))
        {
            //当前位是1, src1左移后累加至结果
            huint352_left_shift(state,state,i-last_index);
            last_index=i;
...
void huint352_div(huint352_t *state,huint352_t *state1,huint352_t *state2,huint352_t *dst,const huint352_t *src1,const huint352_t *src2)
{
    ...
    {
        huint352_right_shift(state1,state1,i-last_index);
        last_index=i;
        if(huint352_compare(state,state1) >= 0)
        {
            //被除数大于左移后的除数，直接相减并将相应位置1
            huint352_sub(state2,state,state1);
