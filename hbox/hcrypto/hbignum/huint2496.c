...
void huint2496_load_be(huint2496_t *num,const uint8_t *bytes,size_t bytes_count)
{
    ...
    {
        //丢弃高位
        size_t bytes_overflow=bytes_count-sizeof(huint2496_t);
...
static size_t huint2496_left_shift_internal(huint2496_t *dst,const huint2496_t *src,size_t bits)
{
    if(dst==NULL || src==NULL)
    {
        return 0;
...
void huint2496_left_shift(huint2496_t *dst,const huint2496_t *src,size_t bits)
{
    ...
    if(bits >= HUINT2496_BITS_COUNT)
    {
        //大于等于大数的位数，直接置0
        huint2496_load_uint32(dst,0);
        return;
    }

    huint2496_copy(dst,src);

    while(bits > 0)
    {
        bits-=huint2496_left_shift_internal(dst,dst,bits);
    }
}

static size_t huint2496_right_shift_internal(huint2496_t *dst,const huint2496_t *src,size_t bits)
{
    if(dst==NULL || src==NULL)
    {
        return 0;
...
void huint2496_right_shift(huint2496_t *dst,const huint2496_t *src,size_t bits)
{
    ...
    while(bits > 0)
    {
        bits-=huint2496_right_shift_internal(dst,dst,bits);
    }
}
...
size_t huint2496_clz(const huint2496_t *dst)
{
    if(dst!=NULL)
    {
        ...
        for(size_t i=index_start; i < HUINT2496_BITS_COUNT; i++)
        {
            if(huint2496_bit(dst,HUINT2496_BITS_COUNT-1-i))
            {
                return i;
            }
        }
...
void huint2496_sub(huint2496_t *dst,const huint2496_t *src1,const huint2496_t *src2)
{
    ...
    //求补码
    huint2496_complement(dst,src2);

    //对补码进行加
    huint2496_add(dst,dst,src1);
}
...
void huint2496_mul_with_stack(huint2496_t *dst,const huint2496_t *src1,const huint2496_t *src2)
{
    ...
    huint2496_mul(&state,dst,src1,src2);
}
...
void huint2496_mul_with_external_state(huint2496_state_t *state,huint2496_t *dst,const huint2496_t *src1,const huint2496_t *src2)
{
    ...
    huint2496_mul(&state->state[0],dst,src1,src2);
}
...
void huint2496_div(huint2496_t *state,huint2496_t *state1,huint2496_t *state2,huint2496_t *dst,const huint2496_t *src1,const huint2496_t *src2)
{
    ...
    huint2496_load_uint32(dst,0);
    if(huint2496_compare(src2,dst)==0 )
    {
        huint2496_load_uint32(state,0);
        //除0错误
        return;
    }

    if(huint2496_compare(src2,src1) > 0)
    {
        //除数大于被除数
        huint2496_copy(state,src1);
        huint2496_load_uint32(dst,0);
        return;
    }

    size_t clz1=huint2496_clz(src1);
    size_t clz2=huint2496_clz(src2);
    if(clz1 > clz2)
    {
        //除数大于被除数
        huint2496_copy(state,src1);
    ...
    huint2496_copy(state,src1);
    size_t last_index=0;
    huint2496_left_shift(state1,src2,(clz2-clz1)-last_index);
    for(size_t i=0; i<= clz2-clz1; i++)
    {
        huint2496_right_shift_internal(state1,state1,i-last_index);
        last_index=i;
        if(huint2496_compare(state,state1) >= 0)
        {
            //被除数大于左移后的除数，直接相减并将相应位置1
            {
                //求补码
                huint2496_complement(state2,state1);
                //对补码进行加
                huint2496_add(state,state,state2);
            }
            huint2496_bit_set(dst,(clz2-clz1)-i);
        }
    }
...
{
    huint2496_t state[3]= {0};
    huint2496_div(&state[0],&state[1],&state[2],dst,src1,src2);
    if(mod!=NULL)
    {
        huint2496_copy(mod,&state[0]);
    }
}
...
void huint2496_div_with_external_state(huint2496_state_t * state,huint2496_t *mod,huint2496_t *dst,const huint2496_t *src1,const huint2496_t *src2)
{
    ...
    huint2496_div(&state->state[0],&state->state[1],&state->state[2],dst,src1,src2);
    if(mod!=NULL)
    {
        huint2496_copy(mod,&state->state[0]);
    }
}
...
void huint2496_power(huint2496_t *state,huint2496_t *state1,huint2496_t *state2,huint2496_t *dst,const huint2496_t *src1,const huint2496_t *src2)
{
    ...
    huint2496_load_uint32(state,0);
    huint2496_load_uint32(dst,1);
    if(huint2496_compare(src1,state)==0 )
    {
        //底数为0
        huint2496_load_uint32(dst,0);
        return;
    }
    if(huint2496_compare(src2,state)==0 )
    {
        //任意数的0次方=1
        huint2496_load_uint32(dst,1);
        return;
    ...
    size_t clz2=huint2496_clz(src2);
    huint2496_copy(state,src1);
    for(size_t i=0; i<(HUINT2496_BITS_COUNT - clz2); i++)
    {
        if(huint2496_bit(src2,i))
        {
            //,结果应当乘上当前state的值(将幂函数的指数按照2进制进行拆分成乘法表达式)
            huint2496_mul(state1,state2,dst,state);
            huint2496_copy(dst,state2);
        }
        //计算state的平方（state的平方相当于state的指数乘2，正好对应src2下一个二进制位）
        huint2496_mul(state1,state2,state,state);
        huint2496_copy(state,state2);
    }
...
{
    huint2496_t state[3]= {0};
    huint2496_power(&state[0],&state[1],&state[2],dst,src1,src2);
}
...
void huint2496_power_with_external_state(huint2496_state_t * state,huint2496_t *dst,const huint2496_t *src1,const huint2496_t *src2)
{
    ...
    huint2496_power(&state->state[0],&state->state[1],&state->state[2],dst,src1,src2);
}
...
void huint2496_root(huint2496_t *state,huint2496_t *state1,huint2496_t *state2,huint2496_t *state3,huint2496_t *state4,huint2496_t *dst,const huint2496_t *src,size_t index)
{
    ...
    huint2496_load_uint32(dst,0);
    huint2496_load_uint64(state4,index);

    size_t dst_max_bit=(HUINT2496_BITS_COUNT-huint2496_clz(src)+index-1)/index;

    if(dst_max_bit*index > HUINT2496_BITS_COUNT)
    {
        dst_max_bit--;
    }
    ...
    {
        huint2496_bit_set(dst,dst_max_bit-i);
        huint2496_power(state,state1,state2,state3,dst,state4);
        int compare_ret=huint2496_compare(state3,src);
        if(compare_ret == 0)
        {
            break;
        }
        if(compare_ret > 0)
        {
            huint2496_bit_clear(dst,dst_max_bit-i);
        }
    }
...
{
    huint2496_t state[5];
    huint2496_root(&state[0],&state[1],&state[2],&state[3],&state[4],dst,src,index);
}
...
void huint2496_root_with_external_state(huint2496_state_t * state,huint2496_t *dst,const huint2496_t *src,size_t index)
{
    ...
    huint2496_root(&state->state[0],&state->state[1],&state->state[2],&state->state[3],&state->state[4],dst,src,index);
}
...
void huint2496_power_mod(huint2496_t *state,huint2496_t *state1,huint2496_t *state2,huint2496_t *state3,huint2496_t *dst,const huint2496_t *src1,const huint2496_t *src2,const huint2496_t *src3)
{
    ...
    for(size_t i=0; i<(HUINT2496_BITS_COUNT - clz2); i++)
    {
        if(huint2496_bit(src2,i))
        {
            ...
            huint2496_div(dst,state1,state2,state3,dst,src3);
        }
        //计算state的平方（state的平方相当于state的指数乘2，正好对应src2下一个二进制位）
        huint2496_mul(state1,state2,state,state);
        huint2496_copy(state,state2);
        ...
        huint2496_div(state,state1,state2,state3,state,src3);
    }
...
{
    huint2496_t state[4]= {0};
    huint2496_power_mod(&state[0],&state[1],&state[2],&state[3],dst,src1,src2,src3);
}
...
void huint2496_power_mod_with_external_state(huint2496_state_t * state,huint2496_t *dst,const huint2496_t *src1,const huint2496_t *src2,const huint2496_t *src3)
{
    ...
    huint2496_power_mod(&state->state[0],&state->state[1],&state->state[2],&state->state[3],dst,src1,src2,src3);
}
...
void huint2496_gcd(huint2496_state_t * state,huint2496_t *dst,const huint2496_t *src1,const huint2496_t *src2)
{
     ...
     * 寄存器6,7分别存储除数与被除数
     */
    if(huint2496_compare(src1,src2) >=0)
    {
        huint2496_copy(&state->state[7],src1);
        huint2496_copy(&state->state[6],src2);
    }
    else
    {
        huint2496_copy(&state->state[6],src1);
        huint2496_copy(&state->state[7],src2);
    }
    /*
     * 寄存器4,5分别存储商与余数
     */
    huint2496_load_uint32(&state->state[5],1);//初始时余数不为0
    /*
     * 寄存器3为0寄存器
     */
    huint2496_load_uint32(&state->state[3],0);
    if(huint2496_compare(&state->state[3],&state->state[6])==0)
    {
        /*
         * 当其中一个数为0时,返回较大的数
         */
        huint2496_copy(dst,&state->state[7]);
        return;
    }

    do
    {
        huint2496_div(&state->state[5],&state->state[0],&state->state[1],&state->state[4],&state->state[7],&state->state[6]);
        /*
         * 重新设置被除数与除数
         */
        huint2496_copy(&state->state[7],&state->state[6]);
        huint2496_copy(&state->state[6],&state->state[5]);
    }
    while(huint2496_compare(&state->state[3],&state->state[5])!=0);
...
size_t huint2496_dec_number_count(huint2496_state_t * state,const huint2496_t *src)
{
    ...
    while(!is_ok)
    {
         ...
         */
        huint2496_load_uint32(&state->state[6],10);
        huint2496_load_uint32(&state->state[7],dec_number_count);
        /*
         * 寄存器5存储10的指数的结果
         */
        huint2496_power_with_external_state(state,&state->state[5],&state->state[6],&state->state[7]);

        /*
         * 比较大小
         */
        switch(huint2496_compare(&state->state[5],src))
        {
        case 1:
        {
            is_ok=true;
...
size_t huint2496_dec_number(huint2496_state_t * state,const huint2496_t *src,size_t index)
{
     ...
     */
    huint2496_load_uint32(&state->state[6],10);
    huint2496_load_uint32(&state->state[7],index+1);
    /*
     * 寄存器5存储10的指数的结果
     */
    huint2496_power_with_external_state(state,&state->state[5],&state->state[6],&state->state[7]);
     ...
     * 第一次除法
     */
    huint2496_copy(&state->state[7],&state->state[5]);
    huint2496_div_with_external_state(state,&state->state[4],&state->state[5],src,&state->state[7]);

    /*
     * 寄存器6存储10，寄存器7存储10的指数
     */
    huint2496_load_uint32(&state->state[7],index);
    huint2496_power_with_external_state(state,&state->state[5],&state->state[6],&state->state[7]);

    /*
     * 第二次除法
     */
    huint2496_copy(&state->state[6],&state->state[4]);
    huint2496_copy(&state->state[7],&state->state[5]);
    huint2496_div_with_external_state(state,&state->state[4],&state->state[5],&state->state[6],&state->state[7]);

    uint32_t result=0;
    huint2496_store_uint32(&state->state[5],&result);
    dec_number=result;
