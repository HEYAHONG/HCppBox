...
size_t huint2496_clz(const huint2496_t *dst)
{
    if(dst!=NULL)
    {
        for(size_t i=0; i < HUINT2496_BITS_COUNT; i++)
        {
...
void huint2496_mul(huint2496_t *state,huint2496_t *dst,const huint2496_t *src1,const huint2496_t *src2)
{
    ...
    huint2496_load_uint32(dst,0);
    size_t clz=huint2496_clz(src2);
    size_t ctz=huint2496_ctz(src2);
    size_t last_index=ctz;
    huint2496_left_shift(state,src1,last_index);
    for(size_t i=ctz; i < (HUINT2496_BITS_COUNT-clz); i++)
    {
        if(huint2496_bit(src2,i))
        {
            //当前位是1, src1左移后累加至结果
            huint2496_left_shift(state,state,i-last_index);
            last_index=i;
...
void huint2496_div(huint2496_t *state,huint2496_t *state1,huint2496_t *state2,huint2496_t *dst,const huint2496_t *src1,const huint2496_t *src2)
{
    ...
    {
        huint2496_right_shift(state1,state1,i-last_index);
        last_index=i;
        if(huint2496_compare(state,state1) >= 0)
        {
            //被除数大于左移后的除数，直接相减并将相应位置1
            huint2496_sub(state2,state,state1);
