...
size_t huint128_clz(const huint128_t *dst)
{
    if(dst!=NULL)
    {
        ...
        for(size_t i=index_start; i < HUINT128_BITS_COUNT; i++)
        {
...
bool huint128_t_is_zero(const huint128_t * src)
{
    bool ret=false;
    if(src==NULL)
    {
...
bool huint128_t_is_one(const huint128_t * src)
{
    bool ret=false;
    if(src==NULL)
    {
...
void huint128_mul_with_stack(huint128_t *dst,const huint128_t *src1,const huint128_t *src2)
{
    huint128_t state;

    huint128_mul(&state,dst,src1,src2);
}

void huint128_mul_with_external_state(huint128_state_t *state,huint128_t *dst,const huint128_t *src1,const huint128_t *src2)
{
    if(state==NULL)
    {
        return;
    ...
    huint128_mul(&state->state[0],dst,src1,src2);
}
...
void huint128_div(huint128_t *state,huint128_t *state1,huint128_t *state2,huint128_t *dst,const huint128_t *src1,const huint128_t *src2)
{
    ...
    huint128_load_uint32(dst,0);
    if(huint128_compare(src2,dst)==0 )
    {
    ...
    huint128_copy(state,src1);
    size_t last_index=0;
    huint128_left_shift(state1,src2,(clz2-clz1)-last_index);
    for(size_t i=0; i<= clz2-clz1; i++)
    {
        huint128_right_shift_internal(state1,state1,i-last_index);
        last_index=i;
        if(huint128_compare(state,state1) >= 0)
        {
            ...
            {
                //求补码
                huint128_complement(state2,state1);
                //对补码进行加
                huint128_add(state,state,state2);
            }
...
void huint128_div_with_stack(huint128_t *mod,huint128_t *dst,const huint128_t *src1,const huint128_t *src2)
{
    huint128_t state[3]= {0};
    huint128_div(&state[0],&state[1],&state[2],dst,src1,src2);
    if(mod!=NULL)
    {
        huint128_copy(mod,&state[0]);
    }
}

void huint128_div_with_external_state(huint128_state_t * state,huint128_t *mod,huint128_t *dst,const huint128_t *src1,const huint128_t *src2)
{
    if(state==NULL)
    {
        return;
    }
    huint128_div(&state->state[0],&state->state[1],&state->state[2],dst,src1,src2);
    if(mod!=NULL)
    {
        huint128_copy(mod,&state->state[0]);
    }
}
...
void huint128_power_with_stack(huint128_t *dst,const huint128_t *src1,const huint128_t *src2)
{
    huint128_t state[3]= {0};
    huint128_power(&state[0],&state[1],&state[2],dst,src1,src2);
}

void huint128_power_with_external_state(huint128_state_t * state,huint128_t *dst,const huint128_t *src1,const huint128_t *src2)
{
    if(state==NULL)
    {
        return;
    }
    huint128_power(&state->state[0],&state->state[1],&state->state[2],dst,src1,src2);
}

void huint128_root(huint128_t *state,huint128_t *state1,huint128_t *state2,huint128_t *state3,huint128_t *state4,huint128_t *dst,const huint128_t *src,size_t index)
{
    ...
    huint128_load_uint32(dst,0);
    huint128_load_uint64(state4,index);

    size_t dst_max_bit=(HUINT128_BITS_COUNT-huint128_clz(src)+index-1)/index;

    if(dst_max_bit*index > HUINT128_BITS_COUNT)
    {
        dst_max_bit--;
    }
    ...
    {
        huint128_bit_set(dst,dst_max_bit-i);
        huint128_power(state,state1,state2,state3,dst,state4);
        int compare_ret=huint128_compare(state3,src);
        if(compare_ret == 0)
        {
            break;
        }
        if(compare_ret > 0)
        {
            huint128_bit_clear(dst,dst_max_bit-i);
        }
    }

}

void huint128_root_with_stack(huint128_t *dst,const huint128_t *src,size_t index)
{
    huint128_t state[5];
    huint128_root(&state[0],&state[1],&state[2],&state[3],&state[4],dst,src,index);
}

void huint128_root_with_external_state(huint128_state_t * state,huint128_t *dst,const huint128_t *src,size_t index)
{
    if(state==NULL)
    {
        return;
    }
    huint128_root(&state->state[0],&state->state[1],&state->state[2],&state->state[3],&state->state[4],dst,src,index);
}
...
void huint128_power_mod_with_stack(huint128_t *dst,const huint128_t *src1,const huint128_t *src2,const huint128_t *src3)
{
    huint128_t state[4]= {0};
    huint128_power_mod(&state[0],&state[1],&state[2],&state[3],dst,src1,src2,src3);
}

void huint128_power_mod_with_external_state(huint128_state_t * state,huint128_t *dst,const huint128_t *src1,const huint128_t *src2,const huint128_t *src3)
{
    if(state==NULL)
    {
        return;
    }
    huint128_power_mod(&state->state[0],&state->state[1],&state->state[2],&state->state[3],dst,src1,src2,src3);
}

void huint128_gcd(huint128_state_t * state,huint128_t *dst,const huint128_t *src1,const huint128_t *src2)
{
    if(state==NULL || dst==NULL || src1==NULL || src2==NULL)
    {
        return;
     ...
     * 寄存器6,7分别存储除数与被除数
     */
    if(huint128_compare(src1,src2) >=0)
    {
        huint128_copy(&state->state[7],src1);
        huint128_copy(&state->state[6],src2);
    }
    else
    {
        huint128_copy(&state->state[6],src1);
        huint128_copy(&state->state[7],src2);
    }
    /*
     * 寄存器4,5分别存储商与余数
     */
    huint128_load_uint32(&state->state[5],1);//初始时余数不为0
    /*
     * 寄存器3为0寄存器
     */
    huint128_load_uint32(&state->state[3],0);
    if(huint128_compare(&state->state[3],&state->state[6])==0)
    {
        /*
         * 当其中一个数为0时,返回较大的数
         */
        huint128_copy(dst,&state->state[7]);
        return;
    }

    do
    {
        huint128_div(&state->state[5],&state->state[0],&state->state[1],&state->state[4],&state->state[7],&state->state[6]);
        /*
         * 重新设置被除数与除数
         */
        huint128_copy(&state->state[7],&state->state[6]);
        huint128_copy(&state->state[6],&state->state[5]);
    }
    while(huint128_compare(&state->state[3],&state->state[5])!=0);
