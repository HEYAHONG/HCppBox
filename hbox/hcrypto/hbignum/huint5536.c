...
size_t huint5536_clz(const huint5536_t *dst)
{
    if(dst!=NULL)
    {
        for(size_t i=0; i < HUINT5536_BITS_COUNT; i++)
        {
...
void huint5536_mul(huint5536_t *state,huint5536_t *dst,const huint5536_t *src1,const huint5536_t *src2)
{
    ...
    huint5536_load_uint32(dst,0);
    size_t clz=huint5536_clz(src2);
    size_t ctz=huint5536_ctz(src2);
    size_t last_index=ctz;
    huint5536_left_shift(state,src1,last_index);
    for(size_t i=ctz; i < (HUINT5536_BITS_COUNT-clz); i++)
    {
        if(huint5536_bit(src2,i))
        {
            //当前位是1, src1左移后累加至结果
            huint5536_left_shift(state,state,i-last_index);
            last_index=i;
...
void huint5536_div(huint5536_t *state,huint5536_t *state1,huint5536_t *state2,huint5536_t *dst,const huint5536_t *src1,const huint5536_t *src2)
{
    ...
    {
        huint5536_right_shift(state1,state1,i-last_index);
        last_index=i;
        if(huint5536_compare(state,state1) >= 0)
        {
            //被除数大于左移后的除数，直接相减并将相应位置1
            huint5536_sub(state2,state,state1);
