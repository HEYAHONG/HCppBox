...
size_t huint1248_clz(const huint1248_t *dst)
{
    if(dst!=NULL)
    {
        for(size_t i=0; i < HUINT1248_BITS_COUNT; i++)
        {
...
void huint1248_mul(huint1248_t *state,huint1248_t *dst,const huint1248_t *src1,const huint1248_t *src2)
{
    ...
    huint1248_load_uint32(dst,0);
    size_t clz=huint1248_clz(src2);
    size_t ctz=huint1248_ctz(src2);
    size_t last_index=ctz;
    huint1248_left_shift(state,src1,last_index);
    for(size_t i=ctz; i < (HUINT1248_BITS_COUNT-clz); i++)
    {
        if(huint1248_bit(src2,i))
        {
            //当前位是1, src1左移后累加至结果
            huint1248_left_shift(state,state,i-last_index);
            last_index=i;
...
void huint1248_div(huint1248_t *state,huint1248_t *state1,huint1248_t *state2,huint1248_t *dst,const huint1248_t *src1,const huint1248_t *src2)
{
    ...
    {
        huint1248_right_shift(state1,state1,i-last_index);
        last_index=i;
        if(huint1248_compare(state,state1) >= 0)
        {
            //被除数大于左移后的除数，直接相减并将相应位置1
            huint1248_sub(state2,state,state1);
