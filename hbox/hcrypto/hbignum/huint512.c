...
size_t huint512_clz(const huint512_t *dst)
{
    if(dst!=NULL)
    {
        ...
        for(size_t i=index_start; i < HUINT512_BITS_COUNT; i++)
        {
...
bool huint512_t_is_zero(const huint512_t * src)
{
    bool ret=false;
    if(src==NULL)
    {
...
bool huint512_t_is_one(const huint512_t * src)
{
    bool ret=false;
    if(src==NULL)
    {
...
void huint512_mul_with_stack(huint512_t *dst,const huint512_t *src1,const huint512_t *src2)
{
    huint512_t state;

    huint512_mul(&state,dst,src1,src2);
}

void huint512_mul_with_external_state(huint512_state_t *state,huint512_t *dst,const huint512_t *src1,const huint512_t *src2)
{
    if(state==NULL)
    {
        return;
    ...
    huint512_mul(&state->state[0],dst,src1,src2);
}
...
void huint512_div(huint512_t *state,huint512_t *state1,huint512_t *state2,huint512_t *dst,const huint512_t *src1,const huint512_t *src2)
{
    ...
    huint512_load_uint32(dst,0);
    if(huint512_compare(src2,dst)==0 )
    {
    ...
    huint512_copy(state,src1);
    size_t last_index=0;
    huint512_left_shift(state1,src2,(clz2-clz1)-last_index);
    for(size_t i=0; i<= clz2-clz1; i++)
    {
        huint512_right_shift_internal(state1,state1,i-last_index);
        last_index=i;
        if(huint512_compare(state,state1) >= 0)
        {
            ...
            {
                //求补码
                huint512_complement(state2,state1);
                //对补码进行加
                huint512_add(state,state,state2);
            }
...
void huint512_div_with_stack(huint512_t *mod,huint512_t *dst,const huint512_t *src1,const huint512_t *src2)
{
    huint512_t state[3]= {0};
    huint512_div(&state[0],&state[1],&state[2],dst,src1,src2);
    if(mod!=NULL)
    {
        huint512_copy(mod,&state[0]);
    }
}

void huint512_div_with_external_state(huint512_state_t * state,huint512_t *mod,huint512_t *dst,const huint512_t *src1,const huint512_t *src2)
{
    if(state==NULL)
    {
        return;
    }
    huint512_div(&state->state[0],&state->state[1],&state->state[2],dst,src1,src2);
    if(mod!=NULL)
    {
        huint512_copy(mod,&state->state[0]);
    }
}
...
void huint512_power_with_stack(huint512_t *dst,const huint512_t *src1,const huint512_t *src2)
{
    huint512_t state[3]= {0};
    huint512_power(&state[0],&state[1],&state[2],dst,src1,src2);
}

void huint512_power_with_external_state(huint512_state_t * state,huint512_t *dst,const huint512_t *src1,const huint512_t *src2)
{
    if(state==NULL)
    {
        return;
    }
    huint512_power(&state->state[0],&state->state[1],&state->state[2],dst,src1,src2);
}

void huint512_root(huint512_t *state,huint512_t *state1,huint512_t *state2,huint512_t *state3,huint512_t *state4,huint512_t *dst,const huint512_t *src,size_t index)
{
    ...
    huint512_load_uint32(dst,0);
    huint512_load_uint64(state4,index);

    size_t dst_max_bit=(HUINT512_BITS_COUNT-huint512_clz(src)+index-1)/index;

    if(dst_max_bit*index > HUINT512_BITS_COUNT)
    {
        dst_max_bit--;
    }
    ...
    {
        huint512_bit_set(dst,dst_max_bit-i);
        huint512_power(state,state1,state2,state3,dst,state4);
        int compare_ret=huint512_compare(state3,src);
        if(compare_ret == 0)
        {
            break;
        }
        if(compare_ret > 0)
        {
            huint512_bit_clear(dst,dst_max_bit-i);
        }
    }

}

void huint512_root_with_stack(huint512_t *dst,const huint512_t *src,size_t index)
{
    huint512_t state[5];
    huint512_root(&state[0],&state[1],&state[2],&state[3],&state[4],dst,src,index);
}

void huint512_root_with_external_state(huint512_state_t * state,huint512_t *dst,const huint512_t *src,size_t index)
{
    if(state==NULL)
    {
        return;
    }
    huint512_root(&state->state[0],&state->state[1],&state->state[2],&state->state[3],&state->state[4],dst,src,index);
}
...
void huint512_power_mod_with_stack(huint512_t *dst,const huint512_t *src1,const huint512_t *src2,const huint512_t *src3)
{
    huint512_t state[4]= {0};
    huint512_power_mod(&state[0],&state[1],&state[2],&state[3],dst,src1,src2,src3);
}

void huint512_power_mod_with_external_state(huint512_state_t * state,huint512_t *dst,const huint512_t *src1,const huint512_t *src2,const huint512_t *src3)
{
    if(state==NULL)
    {
        return;
    }
    huint512_power_mod(&state->state[0],&state->state[1],&state->state[2],&state->state[3],dst,src1,src2,src3);
}

void huint512_gcd(huint512_state_t * state,huint512_t *dst,const huint512_t *src1,const huint512_t *src2)
{
    if(state==NULL || dst==NULL || src1==NULL || src2==NULL)
    {
        return;
     ...
     * 寄存器6,7分别存储除数与被除数
     */
    if(huint512_compare(src1,src2) >=0)
    {
        huint512_copy(&state->state[7],src1);
        huint512_copy(&state->state[6],src2);
    }
    else
    {
        huint512_copy(&state->state[6],src1);
        huint512_copy(&state->state[7],src2);
    }
    /*
     * 寄存器4,5分别存储商与余数
     */
    huint512_load_uint32(&state->state[5],1);//初始时余数不为0
    /*
     * 寄存器3为0寄存器
     */
    huint512_load_uint32(&state->state[3],0);
    if(huint512_compare(&state->state[3],&state->state[6])==0)
    {
        /*
         * 当其中一个数为0时,返回较大的数
         */
        huint512_copy(dst,&state->state[7]);
        return;
    }

    do
    {
        huint512_div(&state->state[5],&state->state[0],&state->state[1],&state->state[4],&state->state[7],&state->state[6]);
        /*
         * 重新设置被除数与除数
         */
        huint512_copy(&state->state[7],&state->state[6]);
        huint512_copy(&state->state[6],&state->state[5]);
    }
    while(huint512_compare(&state->state[3],&state->state[5])!=0);
...
size_t huint512_dec_number_count(huint512_state_t * state,const huint512_t *src)
{
    ...
    while(!is_ok)
    {
         ...
         */
        huint512_load_uint32(&state->state[6],10);
        huint512_load_uint32(&state->state[7],dec_number_count);
        /*
         * 寄存器5存储10的指数的结果
         */
        huint512_power_with_external_state(state,&state->state[5],&state->state[6],&state->state[7]);

        /*
         * 比较大小
         */
        switch(huint512_compare(&state->state[5],src))
        {
        case 1:
        {
            is_ok=true;
...
size_t huint512_dec_number(huint512_state_t * state,const huint512_t *src,size_t index)
{
    size_t dec_number=0;
    if(state==NULL || src==NULL)
    {
     ...
     */
    huint512_load_uint32(&state->state[6],10);
    huint512_load_uint32(&state->state[7],index+1);
    /*
     * 寄存器5存储10的指数的结果
     */
    huint512_power_with_external_state(state,&state->state[5],&state->state[6],&state->state[7]);
     ...
     * 第一次除法
     */
    huint512_copy(&state->state[7],&state->state[5]);
    huint512_div_with_external_state(state,&state->state[4],&state->state[5],src,&state->state[7]);

    /*
     * 寄存器6存储10，寄存器7存储10的指数
     */
    huint512_load_uint32(&state->state[7],index);
    huint512_power_with_external_state(state,&state->state[5],&state->state[6],&state->state[7]);

    /*
     * 第二次除法
     */
    huint512_copy(&state->state[6],&state->state[4]);
    huint512_copy(&state->state[7],&state->state[5]);
    huint512_div_with_external_state(state,&state->state[4],&state->state[5],&state->state[6],&state->state[7]);

    uint32_t result=0;
    huint512_store_uint32(&state->state[5],&result);
    dec_number=result;
