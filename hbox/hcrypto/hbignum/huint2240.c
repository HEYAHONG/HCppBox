...
void huint2240_load_be(huint2240_t *num,const uint8_t *bytes,size_t bytes_count)
{
    ...
    {
        //丢弃高位
        size_t bytes_overflow=bytes_count-sizeof(huint2240_t);
...
static size_t huint2240_left_shift_internal(huint2240_t *dst,const huint2240_t *src,size_t bits)
{
    if(dst==NULL || src==NULL)
    {
        return 0;
...
void huint2240_left_shift(huint2240_t *dst,const huint2240_t *src,size_t bits)
{
    ...
    while(bits > 0)
    {
        bits-=huint2240_left_shift_internal(dst,dst,bits);
    }
}

static size_t huint2240_right_shift_internal(huint2240_t *dst,const huint2240_t *src,size_t bits)
{
    if(dst==NULL || src==NULL)
    {
        return 0;
...
void huint2240_right_shift(huint2240_t *dst,const huint2240_t *src,size_t bits)
{
    ...
    while(bits > 0)
    {
        bits-=huint2240_right_shift_internal(dst,dst,bits);
    }
}
...
size_t huint2240_clz(const huint2240_t *dst)
{
    if(dst!=NULL)
    {
        for(size_t i=0; i < HUINT2240_BITS_COUNT; i++)
        {
            if(huint2240_bit(dst,HUINT2240_BITS_COUNT-1-i))
            {
                return i;
            }
        }
...
bool huint2240_t_is_zero(const huint2240_t * src)
{
    bool ret=false;
    if(src==NULL)
    {
...
bool huint2240_t_is_one(const huint2240_t * src)
{
    bool ret=false;
    if(src==NULL)
    {
...
void huint2240_sub(huint2240_t *dst,const huint2240_t *src1,const huint2240_t *src2)
{
    ...
    //求补码
    huint2240_complement(dst,src2);

    //对补码进行加
    huint2240_add(dst,dst,src1);
}
...
void huint2240_mul(huint2240_t *state,huint2240_t *dst,const huint2240_t *src1,const huint2240_t *src2)
{
    ...
    huint2240_load_uint32(dst,0);
    size_t clz=huint2240_clz(src2);
    size_t ctz=huint2240_ctz(src2);
    size_t last_index=ctz;
    huint2240_left_shift(state,src1,last_index);
    for(size_t i=ctz; i < (HUINT2240_BITS_COUNT-clz); i++)
    {
        if(huint2240_bit(src2,i))
        {
            //当前位是1, src1左移后累加至结果
            huint2240_left_shift(state,state,i-last_index);
            last_index=i;
            huint2240_add(dst,dst,state);
        }
    }
}

void huint2240_mul_with_stack(huint2240_t *dst,const huint2240_t *src1,const huint2240_t *src2)
{
    huint2240_t state;

    huint2240_mul(&state,dst,src1,src2);
}

void huint2240_mul_with_external_state(huint2240_state_t *state,huint2240_t *dst,const huint2240_t *src1,const huint2240_t *src2)
{
    if(state==NULL)
    {
        return;
    ...
    huint2240_mul(&state->state[0],dst,src1,src2);
}
...
void huint2240_div(huint2240_t *state,huint2240_t *state1,huint2240_t *state2,huint2240_t *dst,const huint2240_t *src1,const huint2240_t *src2)
{
    ...
    huint2240_load_uint32(dst,0);
    if(huint2240_compare(src2,dst)==0 )
    {
        huint2240_load_uint32(state,0);
        //除0错误
        return;
    }

    if(huint2240_compare(src2,src1) > 0)
    {
        //除数大于被除数
        huint2240_copy(state,src1);
        huint2240_load_uint32(dst,0);
        return;
    }

    size_t clz1=huint2240_clz(src1);
    size_t clz2=huint2240_clz(src2);
    if(clz1 > clz2)
    {
        //除数大于被除数
        huint2240_copy(state,src1);
    ...
    huint2240_copy(state,src1);
    size_t last_index=0;
    huint2240_left_shift(state1,src2,(clz2-clz1)-last_index);
    for(size_t i=0; i<= clz2-clz1; i++)
    {
        huint2240_right_shift(state1,state1,i-last_index);
        last_index=i;
        if(huint2240_compare(state,state1) >= 0)
        {
            //被除数大于左移后的除数，直接相减并将相应位置1
            huint2240_sub(state2,state,state1);
            huint2240_bit_set(dst,(clz2-clz1)-i);
            //余数保存至state
...
void huint2240_div_with_external_state(huint2240_state_t * state,huint2240_t *mod,huint2240_t *dst,const huint2240_t *src1,const huint2240_t *src2)
{
    if(state==NULL)
    {
        return;
    }
    huint2240_div(&state->state[0],&state->state[1],&state->state[2],dst,src1,src2);
    if(mod!=NULL)
    {
        huint2240_copy(mod,&state->state[0]);
    }
}
...
void huint2240_power(huint2240_t *state,huint2240_t *state1,huint2240_t *state2,huint2240_t *dst,const huint2240_t *src1,const huint2240_t *src2)
{
    ...
    huint2240_load_uint32(state,0);
    huint2240_load_uint32(dst,1);
    if(huint2240_compare(src1,state)==0 )
    {
        //底数为0
        huint2240_load_uint32(dst,0);
        return;
    }
    if(huint2240_compare(src2,state)==0 )
    {
        //任意数的0次方=1
        huint2240_load_uint32(dst,1);
        return;
    ...
    size_t clz2=huint2240_clz(src2);
    huint2240_copy(state,src1);
    for(size_t i=0; i<(HUINT2240_BITS_COUNT - clz2); i++)
    {
        if(huint2240_bit(src2,i))
        {
            //,结果应当乘上当前state的值(将幂函数的指数按照2进制进行拆分成乘法表达式)
            huint2240_mul(state1,state2,dst,state);
            huint2240_copy(dst,state2);
        }
        //计算state的平方（state的平方相当于state的指数乘2，正好对应src2下一个二进制位）
        huint2240_mul(state1,state2,state,state);
        huint2240_copy(state,state2);
    }
...
void huint2240_power_with_external_state(huint2240_state_t * state,huint2240_t *dst,const huint2240_t *src1,const huint2240_t *src2)
{
    if(state==NULL)
    {
        return;
...
void huint2240_power_mod(huint2240_t *state,huint2240_t *state1,huint2240_t *state2,huint2240_t *state3,huint2240_t *dst,const huint2240_t *src1,const huint2240_t *src2,const huint2240_t *src3)
{
    ...
    for(size_t i=0; i<(HUINT2240_BITS_COUNT - clz2); i++)
    {
        if(huint2240_bit(src2,i))
        {
            ...
            huint2240_div(dst,state1,state2,state3,dst,src3);
        }
        //计算state的平方（state的平方相当于state的指数乘2，正好对应src2下一个二进制位）
        huint2240_mul(state1,state2,state,state);
        huint2240_copy(state,state2);
...
void huint2240_power_mod_with_external_state(huint2240_state_t * state,huint2240_t *dst,const huint2240_t *src1,const huint2240_t *src2,const huint2240_t *src3)
{
    if(state==NULL)
    {
        return;
    }
    huint2240_power_mod(&state->state[0],&state->state[1],&state->state[2],&state->state[3],dst,src1,src2,src3);
}

void huint2240_gcd(huint2240_state_t * state,huint2240_t *dst,const huint2240_t *src1,const huint2240_t *src2)
{
    if(state==NULL || dst==NULL || src1==NULL || src2==NULL)
    {
        return;
     ...
     * 寄存器6,7分别存储除数与被除数
     */
    if(huint2240_compare(src1,src2) >=0)
    {
        huint2240_copy(&state->state[7],src1);
        huint2240_copy(&state->state[6],src2);
    }
    else
    {
        huint2240_copy(&state->state[6],src1);
        huint2240_copy(&state->state[7],src2);
    }
    /*
     * 寄存器4,5分别存储商与余数
     */
    huint2240_load_uint32(&state->state[5],1);//初始时余数不为0
    /*
     * 寄存器3为0寄存器
     */
    huint2240_load_uint32(&state->state[3],0);
    if(huint2240_compare(&state->state[3],&state->state[6])==0)
    {
        /*
         * 当其中一个数为0时,返回较大的数
         */
        huint2240_copy(dst,&state->state[7]);
        return;
    }

    do
    {
        huint2240_div(&state->state[5],&state->state[0],&state->state[1],&state->state[4],&state->state[7],&state->state[6]);
        /*
         * 重新设置被除数与除数
         */
        huint2240_copy(&state->state[7],&state->state[6]);
        huint2240_copy(&state->state[6],&state->state[5]);
    }
    while(huint2240_compare(&state->state[3],&state->state[5])!=0);
...
size_t huint2240_dec_number_count(huint2240_state_t * state,const huint2240_t *src)
{
    ...
    while(!is_ok)
    {
         ...
         */
        huint2240_load_uint32(&state->state[6],10);
        huint2240_load_uint32(&state->state[7],dec_number_count);
        /*
         * 寄存器5存储10的指数的结果
         */
        huint2240_power_with_external_state(state,&state->state[5],&state->state[6],&state->state[7]);

        /*
         * 比较大小
         */
        switch(huint2240_compare(&state->state[5],src))
        {
        case 1:
        {
            is_ok=true;
...
size_t huint2240_dec_number(huint2240_state_t * state,const huint2240_t *src,size_t index)
{
    size_t dec_number=0;
    if(state==NULL || src==NULL)
    {
     ...
     */
    huint2240_load_uint32(&state->state[6],10);
    huint2240_load_uint32(&state->state[7],index+1);
    /*
     * 寄存器5存储10的指数的结果
     */
    huint2240_power_with_external_state(state,&state->state[5],&state->state[6],&state->state[7]);
     ...
     * 第一次除法
     */
    huint2240_copy(&state->state[7],&state->state[5]);
    huint2240_div_with_external_state(state,&state->state[4],&state->state[5],src,&state->state[7]);

    /*
     * 寄存器6存储10，寄存器7存储10的指数
     */
    huint2240_load_uint32(&state->state[7],index);
    huint2240_power_with_external_state(state,&state->state[5],&state->state[6],&state->state[7]);

    /*
     * 第二次除法
     */
    huint2240_copy(&state->state[6],&state->state[4]);
    huint2240_copy(&state->state[7],&state->state[5]);
    huint2240_div_with_external_state(state,&state->state[4],&state->state[5],&state->state[6],&state->state[7]);

    uint32_t result=0;
    huint2240_store_uint32(&state->state[5],&result);
    dec_number=result;
