...
size_t huint4640_clz(const huint4640_t *dst)
{
    if(dst!=NULL)
    {
        for(size_t i=0; i < HUINT4640_BITS_COUNT; i++)
        {
...
void huint4640_mul(huint4640_t *state,huint4640_t *dst,const huint4640_t *src1,const huint4640_t *src2)
{
    ...
    huint4640_load_uint32(dst,0);
    size_t clz=huint4640_clz(src2);
    size_t ctz=huint4640_ctz(src2);
    size_t last_index=ctz;
    huint4640_left_shift(state,src1,last_index);
    for(size_t i=ctz; i < (HUINT4640_BITS_COUNT-clz); i++)
    {
        if(huint4640_bit(src2,i))
        {
            //当前位是1, src1左移后累加至结果
            huint4640_left_shift(state,state,i-last_index);
            last_index=i;
...
void huint4640_div(huint4640_t *state,huint4640_t *state1,huint4640_t *state2,huint4640_t *dst,const huint4640_t *src1,const huint4640_t *src2)
{
    ...
    {
        huint4640_right_shift(state1,state1,i-last_index);
        last_index=i;
        if(huint4640_compare(state,state1) >= 0)
        {
            //被除数大于左移后的除数，直接相减并将相应位置1
            huint4640_sub(state2,state,state1);
