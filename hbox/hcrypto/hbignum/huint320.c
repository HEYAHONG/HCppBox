...
size_t huint320_clz(const huint320_t *dst)
{
    if(dst!=NULL)
    {
        for(size_t i=0; i < HUINT320_BITS_COUNT; i++)
        {
...
void huint320_mul(huint320_t *state,huint320_t *dst,const huint320_t *src1,const huint320_t *src2)
{
    ...
    huint320_load_uint32(dst,0);
    size_t clz=huint320_clz(src2);
    size_t ctz=huint320_ctz(src2);
    size_t last_index=ctz;
    huint320_left_shift(state,src1,last_index);
    for(size_t i=ctz; i < (HUINT320_BITS_COUNT-clz); i++)
    {
        if(huint320_bit(src2,i))
        {
            //当前位是1, src1左移后累加至结果
            huint320_left_shift(state,state,i-last_index);
            last_index=i;
...
void huint320_div(huint320_t *state,huint320_t *state1,huint320_t *state2,huint320_t *dst,const huint320_t *src1,const huint320_t *src2)
{
    ...
    {
        huint320_right_shift(state1,state1,i-last_index);
        last_index=i;
        if(huint320_compare(state,state1) >= 0)
        {
            //被除数大于左移后的除数，直接相减并将相应位置1
            huint320_sub(state2,state,state1);
