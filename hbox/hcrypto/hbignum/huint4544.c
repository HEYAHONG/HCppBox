...
size_t huint4544_clz(const huint4544_t *dst)
{
    if(dst!=NULL)
    {
        for(size_t i=0; i < HUINT4544_BITS_COUNT; i++)
        {
...
void huint4544_mul(huint4544_t *state,huint4544_t *dst,const huint4544_t *src1,const huint4544_t *src2)
{
    ...
    huint4544_load_uint32(dst,0);
    size_t clz=huint4544_clz(src2);
    size_t ctz=huint4544_ctz(src2);
    size_t last_index=ctz;
    huint4544_left_shift(state,src1,last_index);
    for(size_t i=ctz; i < (HUINT4544_BITS_COUNT-clz); i++)
    {
        if(huint4544_bit(src2,i))
        {
            //当前位是1, src1左移后累加至结果
            huint4544_left_shift(state,state,i-last_index);
            last_index=i;
...
void huint4544_div(huint4544_t *state,huint4544_t *state1,huint4544_t *state2,huint4544_t *dst,const huint4544_t *src1,const huint4544_t *src2)
{
    ...
    {
        huint4544_right_shift(state1,state1,i-last_index);
        last_index=i;
        if(huint4544_compare(state,state1) >= 0)
        {
            //被除数大于左移后的除数，直接相减并将相应位置1
            huint4544_sub(state2,state,state1);
