...
void huint5312_load_be(huint5312_t *num,const uint8_t *bytes,size_t bytes_count)
{
    ...
    {
        //丢弃高位
        size_t bytes_overflow=bytes_count-sizeof(huint5312_t);
...
static size_t huint5312_left_shift_internal(huint5312_t *dst,const huint5312_t *src,size_t bits)
{
    if(dst==NULL || src==NULL)
    {
        return 0;
...
void huint5312_left_shift(huint5312_t *dst,const huint5312_t *src,size_t bits)
{
    ...
    if(bits >= HUINT5312_BITS_COUNT)
    {
        //大于等于大数的位数，直接置0
        huint5312_load_uint32(dst,0);
        return;
    }

    huint5312_copy(dst,src);

    while(bits > 0)
    {
        bits-=huint5312_left_shift_internal(dst,dst,bits);
    }
}

static size_t huint5312_right_shift_internal(huint5312_t *dst,const huint5312_t *src,size_t bits)
{
    if(dst==NULL || src==NULL)
    {
        return 0;
...
void huint5312_right_shift(huint5312_t *dst,const huint5312_t *src,size_t bits)
{
    ...
    while(bits > 0)
    {
        bits-=huint5312_right_shift_internal(dst,dst,bits);
    }
}
...
size_t huint5312_clz(const huint5312_t *dst)
{
    if(dst!=NULL)
    {
        ...
        for(size_t i=index_start; i < HUINT5312_BITS_COUNT; i++)
        {
            if(huint5312_bit(dst,HUINT5312_BITS_COUNT-1-i))
            {
                return i;
            }
        }
...
bool huint5312_t_is_zero(const huint5312_t * src)
{
    bool ret=false;
    if(src==NULL)
    {
...
bool huint5312_t_is_one(const huint5312_t * src)
{
    bool ret=false;
    if(src==NULL)
    {
...
void huint5312_sub(huint5312_t *dst,const huint5312_t *src1,const huint5312_t *src2)
{
    ...
    //求补码
    huint5312_complement(dst,src2);

    //对补码进行加
    huint5312_add(dst,dst,src1);
}
...
void huint5312_mul(huint5312_t *state,huint5312_t *dst,const huint5312_t *src1,const huint5312_t *src2)
{
    ...
    for(size_t i=min_index; i<=max_index; i++)
    {
        ...
        huint5312_add(dst,dst,state);
    }
}

void huint5312_mul_with_stack(huint5312_t *dst,const huint5312_t *src1,const huint5312_t *src2)
{
    huint5312_t state;

    huint5312_mul(&state,dst,src1,src2);
}

void huint5312_mul_with_external_state(huint5312_state_t *state,huint5312_t *dst,const huint5312_t *src1,const huint5312_t *src2)
{
    if(state==NULL)
    {
        return;
    ...
    huint5312_mul(&state->state[0],dst,src1,src2);
}
...
void huint5312_div(huint5312_t *state,huint5312_t *state1,huint5312_t *state2,huint5312_t *dst,const huint5312_t *src1,const huint5312_t *src2)
{
    ...
    huint5312_load_uint32(dst,0);
    if(huint5312_compare(src2,dst)==0 )
    {
        huint5312_load_uint32(state,0);
        //除0错误
        return;
    }

    if(huint5312_compare(src2,src1) > 0)
    {
        //除数大于被除数
        huint5312_copy(state,src1);
        huint5312_load_uint32(dst,0);
        return;
    }

    size_t clz1=huint5312_clz(src1);
    size_t clz2=huint5312_clz(src2);
    if(clz1 > clz2)
    {
        //除数大于被除数
        huint5312_copy(state,src1);
    ...
    huint5312_copy(state,src1);
    size_t last_index=0;
    huint5312_left_shift(state1,src2,(clz2-clz1)-last_index);
    for(size_t i=0; i<= clz2-clz1; i++)
    {
        huint5312_right_shift_internal(state1,state1,i-last_index);
        last_index=i;
        if(huint5312_compare(state,state1) >= 0)
        {
            //被除数大于左移后的除数，直接相减并将相应位置1
            {
                //求补码
                huint5312_complement(state2,state1);
                //对补码进行加
                huint5312_add(state,state,state2);
            }
            huint5312_bit_set(dst,(clz2-clz1)-i);
        }
    }
...
{
    huint5312_t state[3]= {0};
    huint5312_div(&state[0],&state[1],&state[2],dst,src1,src2);
    if(mod!=NULL)
    {
        huint5312_copy(mod,&state[0]);
    }
}

void huint5312_div_with_external_state(huint5312_state_t * state,huint5312_t *mod,huint5312_t *dst,const huint5312_t *src1,const huint5312_t *src2)
{
    if(state==NULL)
    {
        return;
    }
    huint5312_div(&state->state[0],&state->state[1],&state->state[2],dst,src1,src2);
    if(mod!=NULL)
    {
        huint5312_copy(mod,&state->state[0]);
    }
}
...
void huint5312_power(huint5312_t *state,huint5312_t *state1,huint5312_t *state2,huint5312_t *dst,const huint5312_t *src1,const huint5312_t *src2)
{
    ...
    huint5312_load_uint32(state,0);
    huint5312_load_uint32(dst,1);
    if(huint5312_compare(src1,state)==0 )
    {
        //底数为0
        huint5312_load_uint32(dst,0);
        return;
    }
    if(huint5312_compare(src2,state)==0 )
    {
        //任意数的0次方=1
        huint5312_load_uint32(dst,1);
        return;
    ...
    size_t clz2=huint5312_clz(src2);
    huint5312_copy(state,src1);
    for(size_t i=0; i<(HUINT5312_BITS_COUNT - clz2); i++)
    {
        if(huint5312_bit(src2,i))
        {
            //,结果应当乘上当前state的值(将幂函数的指数按照2进制进行拆分成乘法表达式)
            huint5312_mul(state1,state2,dst,state);
            huint5312_copy(dst,state2);
        }
        //计算state的平方（state的平方相当于state的指数乘2，正好对应src2下一个二进制位）
        huint5312_mul(state1,state2,state,state);
        huint5312_copy(state,state2);
    }
...
{
    huint5312_t state[3]= {0};
    huint5312_power(&state[0],&state[1],&state[2],dst,src1,src2);
}

void huint5312_power_with_external_state(huint5312_state_t * state,huint5312_t *dst,const huint5312_t *src1,const huint5312_t *src2)
{
    if(state==NULL)
    {
        return;
    }
    huint5312_power(&state->state[0],&state->state[1],&state->state[2],dst,src1,src2);
}

void huint5312_root(huint5312_t *state,huint5312_t *state1,huint5312_t *state2,huint5312_t *state3,huint5312_t *state4,huint5312_t *dst,const huint5312_t *src,size_t index)
{
    ...
    huint5312_load_uint32(dst,0);
    huint5312_load_uint64(state4,index);

    size_t dst_max_bit=(HUINT5312_BITS_COUNT-huint5312_clz(src)+index-1)/index;

    if(dst_max_bit*index > HUINT5312_BITS_COUNT)
    {
        dst_max_bit--;
    }
    ...
    {
        huint5312_bit_set(dst,dst_max_bit-i);
        huint5312_power(state,state1,state2,state3,dst,state4);
        int compare_ret=huint5312_compare(state3,src);
        if(compare_ret == 0)
        {
            break;
        }
        if(compare_ret > 0)
        {
            huint5312_bit_clear(dst,dst_max_bit-i);
        }
    }

}

void huint5312_root_with_stack(huint5312_t *dst,const huint5312_t *src,size_t index)
{
    huint5312_t state[5];
    huint5312_root(&state[0],&state[1],&state[2],&state[3],&state[4],dst,src,index);
}

void huint5312_root_with_external_state(huint5312_state_t * state,huint5312_t *dst,const huint5312_t *src,size_t index)
{
    if(state==NULL)
    {
        return;
    }
    huint5312_root(&state->state[0],&state->state[1],&state->state[2],&state->state[3],&state->state[4],dst,src,index);
}
...
void huint5312_power_mod(huint5312_t *state,huint5312_t *state1,huint5312_t *state2,huint5312_t *state3,huint5312_t *dst,const huint5312_t *src1,const huint5312_t *src2,const huint5312_t *src3)
{
    ...
    for(size_t i=0; i<(HUINT5312_BITS_COUNT - clz2); i++)
    {
        if(huint5312_bit(src2,i))
        {
            ...
            huint5312_div(dst,state1,state2,state3,dst,src3);
        }
        //计算state的平方（state的平方相当于state的指数乘2，正好对应src2下一个二进制位）
        huint5312_mul(state1,state2,state,state);
        huint5312_copy(state,state2);
        ...
        huint5312_div(state,state1,state2,state3,state,src3);
    }
...
{
    huint5312_t state[4]= {0};
    huint5312_power_mod(&state[0],&state[1],&state[2],&state[3],dst,src1,src2,src3);
}

void huint5312_power_mod_with_external_state(huint5312_state_t * state,huint5312_t *dst,const huint5312_t *src1,const huint5312_t *src2,const huint5312_t *src3)
{
    if(state==NULL)
    {
        return;
    }
    huint5312_power_mod(&state->state[0],&state->state[1],&state->state[2],&state->state[3],dst,src1,src2,src3);
}

void huint5312_gcd(huint5312_state_t * state,huint5312_t *dst,const huint5312_t *src1,const huint5312_t *src2)
{
    if(state==NULL || dst==NULL || src1==NULL || src2==NULL)
    {
        return;
     ...
     * 寄存器6,7分别存储除数与被除数
     */
    if(huint5312_compare(src1,src2) >=0)
    {
        huint5312_copy(&state->state[7],src1);
        huint5312_copy(&state->state[6],src2);
    }
    else
    {
        huint5312_copy(&state->state[6],src1);
        huint5312_copy(&state->state[7],src2);
    }
    /*
     * 寄存器4,5分别存储商与余数
     */
    huint5312_load_uint32(&state->state[5],1);//初始时余数不为0
    /*
     * 寄存器3为0寄存器
     */
    huint5312_load_uint32(&state->state[3],0);
    if(huint5312_compare(&state->state[3],&state->state[6])==0)
    {
        /*
         * 当其中一个数为0时,返回较大的数
         */
        huint5312_copy(dst,&state->state[7]);
        return;
    }

    do
    {
        huint5312_div(&state->state[5],&state->state[0],&state->state[1],&state->state[4],&state->state[7],&state->state[6]);
        /*
         * 重新设置被除数与除数
         */
        huint5312_copy(&state->state[7],&state->state[6]);
        huint5312_copy(&state->state[6],&state->state[5]);
    }
    while(huint5312_compare(&state->state[3],&state->state[5])!=0);
...
size_t huint5312_dec_number_count(huint5312_state_t * state,const huint5312_t *src)
{
    ...
    while(!is_ok)
    {
         ...
         */
        huint5312_load_uint32(&state->state[6],10);
        huint5312_load_uint32(&state->state[7],dec_number_count);
        /*
         * 寄存器5存储10的指数的结果
         */
        huint5312_power_with_external_state(state,&state->state[5],&state->state[6],&state->state[7]);

        /*
         * 比较大小
         */
        switch(huint5312_compare(&state->state[5],src))
        {
        case 1:
        {
            is_ok=true;
...
size_t huint5312_dec_number(huint5312_state_t * state,const huint5312_t *src,size_t index)
{
    size_t dec_number=0;
    if(state==NULL || src==NULL)
    {
     ...
     */
    huint5312_load_uint32(&state->state[6],10);
    huint5312_load_uint32(&state->state[7],index+1);
    /*
     * 寄存器5存储10的指数的结果
     */
    huint5312_power_with_external_state(state,&state->state[5],&state->state[6],&state->state[7]);
     ...
     * 第一次除法
     */
    huint5312_copy(&state->state[7],&state->state[5]);
    huint5312_div_with_external_state(state,&state->state[4],&state->state[5],src,&state->state[7]);

    /*
     * 寄存器6存储10，寄存器7存储10的指数
     */
    huint5312_load_uint32(&state->state[7],index);
    huint5312_power_with_external_state(state,&state->state[5],&state->state[6],&state->state[7]);

    /*
     * 第二次除法
     */
    huint5312_copy(&state->state[6],&state->state[4]);
    huint5312_copy(&state->state[7],&state->state[5]);
    huint5312_div_with_external_state(state,&state->state[4],&state->state[5],&state->state[6],&state->state[7]);

    uint32_t result=0;
    huint5312_store_uint32(&state->state[5],&result);
    dec_number=result;
