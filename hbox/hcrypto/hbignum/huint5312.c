...
size_t huint5312_clz(const huint5312_t *dst)
{
    if(dst!=NULL)
    {
        for(size_t i=0; i < HUINT5312_BITS_COUNT; i++)
        {
...
void huint5312_mul(huint5312_t *state,huint5312_t *dst,const huint5312_t *src1,const huint5312_t *src2)
{
    ...
    huint5312_load_uint32(dst,0);
    size_t clz=huint5312_clz(src2);
    size_t ctz=huint5312_ctz(src2);
    size_t last_index=ctz;
    huint5312_left_shift(state,src1,last_index);
    for(size_t i=ctz; i < (HUINT5312_BITS_COUNT-clz); i++)
    {
        if(huint5312_bit(src2,i))
        {
            //当前位是1, src1左移后累加至结果
            huint5312_left_shift(state,state,i-last_index);
            last_index=i;
...
void huint5312_div(huint5312_t *state,huint5312_t *state1,huint5312_t *state2,huint5312_t *dst,const huint5312_t *src1,const huint5312_t *src2)
{
    ...
    {
        huint5312_right_shift(state1,state1,i-last_index);
        last_index=i;
        if(huint5312_compare(state,state1) >= 0)
        {
            //被除数大于左移后的除数，直接相减并将相应位置1
            huint5312_sub(state2,state,state1);
