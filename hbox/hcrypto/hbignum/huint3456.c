...
size_t huint3456_clz(const huint3456_t *dst)
{
    if(dst!=NULL)
    {
        for(size_t i=0; i < HUINT3456_BITS_COUNT; i++)
        {
...
void huint3456_mul(huint3456_t *state,huint3456_t *dst,const huint3456_t *src1,const huint3456_t *src2)
{
    ...
    huint3456_load_uint32(dst,0);
    size_t clz=huint3456_clz(src2);
    size_t ctz=huint3456_ctz(src2);
    size_t last_index=ctz;
    huint3456_left_shift(state,src1,last_index);
    for(size_t i=ctz; i < (HUINT3456_BITS_COUNT-clz); i++)
    {
        if(huint3456_bit(src2,i))
        {
            //当前位是1, src1左移后累加至结果
            huint3456_left_shift(state,state,i-last_index);
            last_index=i;
...
void huint3456_div(huint3456_t *state,huint3456_t *state1,huint3456_t *state2,huint3456_t *dst,const huint3456_t *src1,const huint3456_t *src2)
{
    ...
    {
        huint3456_right_shift(state1,state1,i-last_index);
        last_index=i;
        if(huint3456_compare(state,state1) >= 0)
        {
            //被除数大于左移后的除数，直接相减并将相应位置1
            huint3456_sub(state2,state,state1);
