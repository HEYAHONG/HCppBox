...
size_t huint5728_clz(const huint5728_t *dst)
{
    if(dst!=NULL)
    {
        for(size_t i=0; i < HUINT5728_BITS_COUNT; i++)
        {
...
void huint5728_mul(huint5728_t *state,huint5728_t *dst,const huint5728_t *src1,const huint5728_t *src2)
{
    ...
    huint5728_load_uint32(dst,0);
    size_t clz=huint5728_clz(src2);
    size_t ctz=huint5728_ctz(src2);
    size_t last_index=ctz;
    huint5728_left_shift(state,src1,last_index);
    for(size_t i=ctz; i < (HUINT5728_BITS_COUNT-clz); i++)
    {
        if(huint5728_bit(src2,i))
        {
            //当前位是1, src1左移后累加至结果
            huint5728_left_shift(state,state,i-last_index);
            last_index=i;
...
void huint5728_div(huint5728_t *state,huint5728_t *state1,huint5728_t *state2,huint5728_t *dst,const huint5728_t *src1,const huint5728_t *src2)
{
    ...
    {
        huint5728_right_shift(state1,state1,i-last_index);
        last_index=i;
        if(huint5728_compare(state,state1) >= 0)
        {
            //被除数大于左移后的除数，直接相减并将相应位置1
            huint5728_sub(state2,state,state1);
