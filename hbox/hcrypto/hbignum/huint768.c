...
size_t huint768_clz(const huint768_t *dst)
{
    if(dst!=NULL)
    {
        for(size_t i=0; i < HUINT768_BITS_COUNT; i++)
        {
...
void huint768_mul(huint768_t *state,huint768_t *dst,const huint768_t *src1,const huint768_t *src2)
{
    ...
    huint768_load_uint32(dst,0);
    size_t clz=huint768_clz(src2);
    size_t ctz=huint768_ctz(src2);
    size_t last_index=ctz;
    huint768_left_shift(state,src1,last_index);
    for(size_t i=ctz; i < (HUINT768_BITS_COUNT-clz); i++)
    {
        if(huint768_bit(src2,i))
        {
            //当前位是1, src1左移后累加至结果
            huint768_left_shift(state,state,i-last_index);
            last_index=i;
...
void huint768_div(huint768_t *state,huint768_t *state1,huint768_t *state2,huint768_t *dst,const huint768_t *src1,const huint768_t *src2)
{
    ...
    {
        huint768_right_shift(state1,state1,i-last_index);
        last_index=i;
        if(huint768_compare(state,state1) >= 0)
        {
            //被除数大于左移后的除数，直接相减并将相应位置1
            huint768_sub(state2,state,state1);
