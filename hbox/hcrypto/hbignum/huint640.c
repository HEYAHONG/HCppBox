...
size_t huint640_clz(const huint640_t *dst)
{
    if(dst!=NULL)
    {
        for(size_t i=0; i < HUINT640_BITS_COUNT; i++)
        {
...
void huint640_mul(huint640_t *state,huint640_t *dst,const huint640_t *src1,const huint640_t *src2)
{
    ...
    huint640_load_uint32(dst,0);
    size_t clz=huint640_clz(src2);
    size_t ctz=huint640_ctz(src2);
    size_t last_index=ctz;
    huint640_left_shift(state,src1,last_index);
    for(size_t i=ctz; i < (HUINT640_BITS_COUNT-clz); i++)
    {
        if(huint640_bit(src2,i))
        {
            //当前位是1, src1左移后累加至结果
            huint640_left_shift(state,state,i-last_index);
            last_index=i;
...
void huint640_div(huint640_t *state,huint640_t *state1,huint640_t *state2,huint640_t *dst,const huint640_t *src1,const huint640_t *src2)
{
    ...
    {
        huint640_right_shift(state1,state1,i-last_index);
        last_index=i;
        if(huint640_compare(state,state1) >= 0)
        {
            //被除数大于左移后的除数，直接相减并将相应位置1
            huint640_sub(state2,state,state1);
