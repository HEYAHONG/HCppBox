...
size_t huint7712_clz(const huint7712_t *dst)
{
    if(dst!=NULL)
    {
        for(size_t i=0; i < HUINT7712_BITS_COUNT; i++)
        {
...
void huint7712_mul(huint7712_t *state,huint7712_t *dst,const huint7712_t *src1,const huint7712_t *src2)
{
    ...
    huint7712_load_uint32(dst,0);
    size_t clz=huint7712_clz(src2);
    size_t ctz=huint7712_ctz(src2);
    size_t last_index=ctz;
    huint7712_left_shift(state,src1,last_index);
    for(size_t i=ctz; i < (HUINT7712_BITS_COUNT-clz); i++)
    {
        if(huint7712_bit(src2,i))
        {
            //当前位是1, src1左移后累加至结果
            huint7712_left_shift(state,state,i-last_index);
            last_index=i;
...
void huint7712_div(huint7712_t *state,huint7712_t *state1,huint7712_t *state2,huint7712_t *dst,const huint7712_t *src1,const huint7712_t *src2)
{
    ...
    {
        huint7712_right_shift(state1,state1,i-last_index);
        last_index=i;
        if(huint7712_compare(state,state1) >= 0)
        {
            //被除数大于左移后的除数，直接相减并将相应位置1
            huint7712_sub(state2,state,state1);
