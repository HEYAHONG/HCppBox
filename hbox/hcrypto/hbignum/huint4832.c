...
size_t huint4832_clz(const huint4832_t *dst)
{
    if(dst!=NULL)
    {
        for(size_t i=0; i < HUINT4832_BITS_COUNT; i++)
        {
...
void huint4832_mul(huint4832_t *state,huint4832_t *dst,const huint4832_t *src1,const huint4832_t *src2)
{
    ...
    huint4832_load_uint32(dst,0);
    size_t clz=huint4832_clz(src2);
    size_t ctz=huint4832_ctz(src2);
    size_t last_index=ctz;
    huint4832_left_shift(state,src1,last_index);
    for(size_t i=ctz; i < (HUINT4832_BITS_COUNT-clz); i++)
    {
        if(huint4832_bit(src2,i))
        {
            //当前位是1, src1左移后累加至结果
            huint4832_left_shift(state,state,i-last_index);
            last_index=i;
...
void huint4832_div(huint4832_t *state,huint4832_t *state1,huint4832_t *state2,huint4832_t *dst,const huint4832_t *src1,const huint4832_t *src2)
{
    ...
    {
        huint4832_right_shift(state1,state1,i-last_index);
        last_index=i;
        if(huint4832_compare(state,state1) >= 0)
        {
            //被除数大于左移后的除数，直接相减并将相应位置1
            huint4832_sub(state2,state,state1);
