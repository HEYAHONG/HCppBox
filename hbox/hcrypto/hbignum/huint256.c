...
size_t huint256_clz(const huint256_t *dst)
{
    if(dst!=NULL)
    {
        ...
        for(size_t i=index_start; i < HUINT256_BITS_COUNT; i++)
        {
...
bool huint256_t_is_zero(const huint256_t * src)
{
    bool ret=false;
    if(src==NULL)
    {
...
bool huint256_t_is_one(const huint256_t * src)
{
    bool ret=false;
    if(src==NULL)
    {
...
void huint256_mul_with_stack(huint256_t *dst,const huint256_t *src1,const huint256_t *src2)
{
    huint256_t state;

    huint256_mul(&state,dst,src1,src2);
}

void huint256_mul_with_external_state(huint256_state_t *state,huint256_t *dst,const huint256_t *src1,const huint256_t *src2)
{
    if(state==NULL)
    {
        return;
    ...
    huint256_mul(&state->state[0],dst,src1,src2);
}
...
void huint256_div(huint256_t *state,huint256_t *state1,huint256_t *state2,huint256_t *dst,const huint256_t *src1,const huint256_t *src2)
{
    ...
    huint256_load_uint32(dst,0);
    if(huint256_compare(src2,dst)==0 )
    {
    ...
    {
        huint256_right_shift_internal(state1,state1,i-last_index);
        last_index=i;
        if(huint256_compare(state,state1) >= 0)
        {
            ...
            {
                //求补码
                huint256_complement(state2,state1);
                //对补码进行加
                huint256_add(state,state,state2);
            }
...
void huint256_div_with_stack(huint256_t *mod,huint256_t *dst,const huint256_t *src1,const huint256_t *src2)
{
    huint256_t state[3]= {0};
    huint256_div(&state[0],&state[1],&state[2],dst,src1,src2);
    if(mod!=NULL)
    {
        huint256_copy(mod,&state[0]);
    }
}

void huint256_div_with_external_state(huint256_state_t * state,huint256_t *mod,huint256_t *dst,const huint256_t *src1,const huint256_t *src2)
{
    if(state==NULL)
    {
        return;
    }
    huint256_div(&state->state[0],&state->state[1],&state->state[2],dst,src1,src2);
    if(mod!=NULL)
    {
        huint256_copy(mod,&state->state[0]);
    }
}
...
void huint256_power_with_stack(huint256_t *dst,const huint256_t *src1,const huint256_t *src2)
{
    huint256_t state[3]= {0};
    huint256_power(&state[0],&state[1],&state[2],dst,src1,src2);
}

void huint256_power_with_external_state(huint256_state_t * state,huint256_t *dst,const huint256_t *src1,const huint256_t *src2)
{
    if(state==NULL)
    {
        return;
    }
    huint256_power(&state->state[0],&state->state[1],&state->state[2],dst,src1,src2);
}

void huint256_root(huint256_t *state,huint256_t *state1,huint256_t *state2,huint256_t *state3,huint256_t *state4,huint256_t *dst,const huint256_t *src,size_t index)
{
    ...
    huint256_load_uint32(dst,0);
    huint256_load_uint64(state4,index);

    size_t dst_max_bit=(HUINT256_BITS_COUNT-huint256_clz(src)+index-1)/index;

    if(dst_max_bit*index > HUINT256_BITS_COUNT)
    {
        dst_max_bit--;
    }
    ...
    {
        huint256_bit_set(dst,dst_max_bit-i);
        huint256_power(state,state1,state2,state3,dst,state4);
        int compare_ret=huint256_compare(state3,src);
        if(compare_ret == 0)
        {
            break;
        }
        if(compare_ret > 0)
        {
            huint256_bit_clear(dst,dst_max_bit-i);
        }
    }

}

void huint256_root_with_stack(huint256_t *dst,const huint256_t *src,size_t index)
{
    huint256_t state[5];
    huint256_root(&state[0],&state[1],&state[2],&state[3],&state[4],dst,src,index);
}

void huint256_root_with_external_state(huint256_state_t * state,huint256_t *dst,const huint256_t *src,size_t index)
{
    if(state==NULL)
    {
        return;
    }
    huint256_root(&state->state[0],&state->state[1],&state->state[2],&state->state[3],&state->state[4],dst,src,index);
}
...
void huint256_power_mod_with_stack(huint256_t *dst,const huint256_t *src1,const huint256_t *src2,const huint256_t *src3)
{
    huint256_t state[4]= {0};
    huint256_power_mod(&state[0],&state[1],&state[2],&state[3],dst,src1,src2,src3);
}

void huint256_power_mod_with_external_state(huint256_state_t * state,huint256_t *dst,const huint256_t *src1,const huint256_t *src2,const huint256_t *src3)
{
    if(state==NULL)
    {
        return;
    }
    huint256_power_mod(&state->state[0],&state->state[1],&state->state[2],&state->state[3],dst,src1,src2,src3);
}

void huint256_gcd(huint256_state_t * state,huint256_t *dst,const huint256_t *src1,const huint256_t *src2)
{
    if(state==NULL || dst==NULL || src1==NULL || src2==NULL)
    {
        return;
     ...
     * 寄存器6,7分别存储除数与被除数
     */
    if(huint256_compare(src1,src2) >=0)
    {
        huint256_copy(&state->state[7],src1);
        huint256_copy(&state->state[6],src2);
    }
    else
    {
        huint256_copy(&state->state[6],src1);
        huint256_copy(&state->state[7],src2);
    }
    /*
     * 寄存器4,5分别存储商与余数
     */
    huint256_load_uint32(&state->state[5],1);//初始时余数不为0
    /*
     * 寄存器3为0寄存器
     */
    huint256_load_uint32(&state->state[3],0);
    if(huint256_compare(&state->state[3],&state->state[6])==0)
    {
        /*
         * 当其中一个数为0时,返回较大的数
         */
        huint256_copy(dst,&state->state[7]);
        return;
    }

    do
    {
        huint256_div(&state->state[5],&state->state[0],&state->state[1],&state->state[4],&state->state[7],&state->state[6]);
        /*
         * 重新设置被除数与除数
         */
        huint256_copy(&state->state[7],&state->state[6]);
        huint256_copy(&state->state[6],&state->state[5]);
    }
    while(huint256_compare(&state->state[3],&state->state[5])!=0);
...
size_t huint256_dec_number_count(huint256_state_t * state,const huint256_t *src)
{
    ...
    while(!is_ok)
    {
         ...
         */
        huint256_load_uint32(&state->state[6],10);
        huint256_load_uint32(&state->state[7],dec_number_count);
        /*
         * 寄存器5存储10的指数的结果
         */
        huint256_power_with_external_state(state,&state->state[5],&state->state[6],&state->state[7]);

        /*
         * 比较大小
         */
        switch(huint256_compare(&state->state[5],src))
        {
        case 1:
        {
            is_ok=true;
...
size_t huint256_dec_number(huint256_state_t * state,const huint256_t *src,size_t index)
{
    size_t dec_number=0;
    if(state==NULL || src==NULL)
    {
     ...
     */
    huint256_load_uint32(&state->state[6],10);
    huint256_load_uint32(&state->state[7],index+1);
    /*
     * 寄存器5存储10的指数的结果
     */
    huint256_power_with_external_state(state,&state->state[5],&state->state[6],&state->state[7]);
     ...
     * 第一次除法
     */
    huint256_copy(&state->state[7],&state->state[5]);
    huint256_div_with_external_state(state,&state->state[4],&state->state[5],src,&state->state[7]);

    /*
     * 寄存器6存储10，寄存器7存储10的指数
     */
    huint256_load_uint32(&state->state[7],index);
    huint256_power_with_external_state(state,&state->state[5],&state->state[6],&state->state[7]);

    /*
     * 第二次除法
     */
    huint256_copy(&state->state[6],&state->state[4]);
    huint256_copy(&state->state[7],&state->state[5]);
    huint256_div_with_external_state(state,&state->state[4],&state->state[5],&state->state[6],&state->state[7]);

    uint32_t result=0;
    huint256_store_uint32(&state->state[5],&result);
    dec_number=result;
