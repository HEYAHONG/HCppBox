...
void huint1824_load_be(huint1824_t *num,const uint8_t *bytes,size_t bytes_count)
{
    ...
    {
        //丢弃高位
        size_t bytes_overflow=bytes_count-sizeof(huint1824_t);
...
static size_t huint1824_left_shift_internal(huint1824_t *dst,const huint1824_t *src,size_t bits)
{
    if(dst==NULL || src==NULL)
    {
        return 0;
...
void huint1824_left_shift(huint1824_t *dst,const huint1824_t *src,size_t bits)
{
    ...
    if(bits >= HUINT1824_BITS_COUNT)
    {
        //大于等于大数的位数，直接置0
        huint1824_load_uint32(dst,0);
        return;
    }

    huint1824_copy(dst,src);

    while(bits > 0)
    {
        bits-=huint1824_left_shift_internal(dst,dst,bits);
    }
}

static size_t huint1824_right_shift_internal(huint1824_t *dst,const huint1824_t *src,size_t bits)
{
    if(dst==NULL || src==NULL)
    {
        return 0;
...
void huint1824_right_shift(huint1824_t *dst,const huint1824_t *src,size_t bits)
{
    ...
    while(bits > 0)
    {
        bits-=huint1824_right_shift_internal(dst,dst,bits);
    }
}
...
size_t huint1824_clz(const huint1824_t *dst)
{
    if(dst!=NULL)
    {
        ...
        for(size_t i=index_start; i < HUINT1824_BITS_COUNT; i++)
        {
            if(huint1824_bit(dst,HUINT1824_BITS_COUNT-1-i))
            {
                return i;
            }
        }
...
void huint1824_sub(huint1824_t *dst,const huint1824_t *src1,const huint1824_t *src2)
{
    ...
    //求补码
    huint1824_complement(dst,src2);

    //对补码进行加
    huint1824_add(dst,dst,src1);
}
...
void huint1824_mul_with_stack(huint1824_t *dst,const huint1824_t *src1,const huint1824_t *src2)
{
    ...
    huint1824_mul(&state,dst,src1,src2);
}
...
void huint1824_mul_with_external_state(huint1824_state_t *state,huint1824_t *dst,const huint1824_t *src1,const huint1824_t *src2)
{
    ...
    huint1824_mul(&state->state[0],dst,src1,src2);
}
...
void huint1824_div(huint1824_t *state,huint1824_t *state1,huint1824_t *state2,huint1824_t *dst,const huint1824_t *src1,const huint1824_t *src2)
{
    ...
    huint1824_load_uint32(dst,0);
    if(huint1824_compare(src2,dst)==0 )
    {
        huint1824_load_uint32(state,0);
        //除0错误
        return;
    }

    if(huint1824_compare(src2,src1) > 0)
    {
        //除数大于被除数
        huint1824_copy(state,src1);
        huint1824_load_uint32(dst,0);
        return;
    }

    size_t clz1=huint1824_clz(src1);
    size_t clz2=huint1824_clz(src2);
    if(clz1 > clz2)
    {
        //除数大于被除数
        huint1824_copy(state,src1);
    ...
    huint1824_copy(state,src1);
    size_t last_index=0;
    huint1824_left_shift(state1,src2,(clz2-clz1)-last_index);
    for(size_t i=0; i<= clz2-clz1; i++)
    {
        huint1824_right_shift_internal(state1,state1,i-last_index);
        last_index=i;
        if(huint1824_compare(state,state1) >= 0)
        {
            //被除数大于左移后的除数，直接相减并将相应位置1
            {
                //求补码
                huint1824_complement(state2,state1);
                //对补码进行加
                huint1824_add(state,state,state2);
            }
            huint1824_bit_set(dst,(clz2-clz1)-i);
        }
    }
...
{
    huint1824_t state[3]= {0};
    huint1824_div(&state[0],&state[1],&state[2],dst,src1,src2);
    if(mod!=NULL)
    {
        huint1824_copy(mod,&state[0]);
    }
}
...
void huint1824_div_with_external_state(huint1824_state_t * state,huint1824_t *mod,huint1824_t *dst,const huint1824_t *src1,const huint1824_t *src2)
{
    ...
    huint1824_div(&state->state[0],&state->state[1],&state->state[2],dst,src1,src2);
    if(mod!=NULL)
    {
        huint1824_copy(mod,&state->state[0]);
    }
}
...
void huint1824_power(huint1824_t *state,huint1824_t *state1,huint1824_t *state2,huint1824_t *dst,const huint1824_t *src1,const huint1824_t *src2)
{
    ...
    huint1824_load_uint32(state,0);
    huint1824_load_uint32(dst,1);
    if(huint1824_compare(src1,state)==0 )
    {
        //底数为0
        huint1824_load_uint32(dst,0);
        return;
    }
    if(huint1824_compare(src2,state)==0 )
    {
        //任意数的0次方=1
        huint1824_load_uint32(dst,1);
        return;
    ...
    size_t clz2=huint1824_clz(src2);
    huint1824_copy(state,src1);
    for(size_t i=0; i<(HUINT1824_BITS_COUNT - clz2); i++)
    {
        if(huint1824_bit(src2,i))
        {
            //,结果应当乘上当前state的值(将幂函数的指数按照2进制进行拆分成乘法表达式)
            huint1824_mul(state1,state2,dst,state);
            huint1824_copy(dst,state2);
        }
        //计算state的平方（state的平方相当于state的指数乘2，正好对应src2下一个二进制位）
        huint1824_mul(state1,state2,state,state);
        huint1824_copy(state,state2);
    }
...
{
    huint1824_t state[3]= {0};
    huint1824_power(&state[0],&state[1],&state[2],dst,src1,src2);
}
...
void huint1824_power_with_external_state(huint1824_state_t * state,huint1824_t *dst,const huint1824_t *src1,const huint1824_t *src2)
{
    ...
    huint1824_power(&state->state[0],&state->state[1],&state->state[2],dst,src1,src2);
}
...
void huint1824_root(huint1824_t *state,huint1824_t *state1,huint1824_t *state2,huint1824_t *state3,huint1824_t *state4,huint1824_t *dst,const huint1824_t *src,size_t index)
{
    ...
    huint1824_load_uint32(dst,0);
    huint1824_load_uint64(state4,index);

    size_t dst_max_bit=(HUINT1824_BITS_COUNT-huint1824_clz(src)+index-1)/index;

    if(dst_max_bit*index > HUINT1824_BITS_COUNT)
    {
        dst_max_bit--;
    }
    ...
    {
        huint1824_bit_set(dst,dst_max_bit-i);
        huint1824_power(state,state1,state2,state3,dst,state4);
        int compare_ret=huint1824_compare(state3,src);
        if(compare_ret == 0)
        {
            break;
        }
        if(compare_ret > 0)
        {
            huint1824_bit_clear(dst,dst_max_bit-i);
        }
    }
...
{
    huint1824_t state[5];
    huint1824_root(&state[0],&state[1],&state[2],&state[3],&state[4],dst,src,index);
}
...
void huint1824_root_with_external_state(huint1824_state_t * state,huint1824_t *dst,const huint1824_t *src,size_t index)
{
    ...
    huint1824_root(&state->state[0],&state->state[1],&state->state[2],&state->state[3],&state->state[4],dst,src,index);
}
...
void huint1824_power_mod(huint1824_t *state,huint1824_t *state1,huint1824_t *state2,huint1824_t *state3,huint1824_t *dst,const huint1824_t *src1,const huint1824_t *src2,const huint1824_t *src3)
{
    ...
    for(size_t i=0; i<(HUINT1824_BITS_COUNT - clz2); i++)
    {
        if(huint1824_bit(src2,i))
        {
            ...
            huint1824_div(dst,state1,state2,state3,dst,src3);
        }
        //计算state的平方（state的平方相当于state的指数乘2，正好对应src2下一个二进制位）
        huint1824_mul(state1,state2,state,state);
        huint1824_copy(state,state2);
        ...
        huint1824_div(state,state1,state2,state3,state,src3);
    }
...
{
    huint1824_t state[4]= {0};
    huint1824_power_mod(&state[0],&state[1],&state[2],&state[3],dst,src1,src2,src3);
}
...
void huint1824_power_mod_with_external_state(huint1824_state_t * state,huint1824_t *dst,const huint1824_t *src1,const huint1824_t *src2,const huint1824_t *src3)
{
    ...
    huint1824_power_mod(&state->state[0],&state->state[1],&state->state[2],&state->state[3],dst,src1,src2,src3);
}
...
void huint1824_gcd(huint1824_state_t * state,huint1824_t *dst,const huint1824_t *src1,const huint1824_t *src2)
{
     ...
     * 寄存器6,7分别存储除数与被除数
     */
    if(huint1824_compare(src1,src2) >=0)
    {
        huint1824_copy(&state->state[7],src1);
        huint1824_copy(&state->state[6],src2);
    }
    else
    {
        huint1824_copy(&state->state[6],src1);
        huint1824_copy(&state->state[7],src2);
    }
    /*
     * 寄存器4,5分别存储商与余数
     */
    huint1824_load_uint32(&state->state[5],1);//初始时余数不为0
    /*
     * 寄存器3为0寄存器
     */
    huint1824_load_uint32(&state->state[3],0);
    if(huint1824_compare(&state->state[3],&state->state[6])==0)
    {
        /*
         * 当其中一个数为0时,返回较大的数
         */
        huint1824_copy(dst,&state->state[7]);
        return;
    }

    do
    {
        huint1824_div(&state->state[5],&state->state[0],&state->state[1],&state->state[4],&state->state[7],&state->state[6]);
        /*
         * 重新设置被除数与除数
         */
        huint1824_copy(&state->state[7],&state->state[6]);
        huint1824_copy(&state->state[6],&state->state[5]);
    }
    while(huint1824_compare(&state->state[3],&state->state[5])!=0);
...
size_t huint1824_dec_number_count(huint1824_state_t * state,const huint1824_t *src)
{
    ...
    while(!is_ok)
    {
         ...
         */
        huint1824_load_uint32(&state->state[6],10);
        huint1824_load_uint32(&state->state[7],dec_number_count);
        /*
         * 寄存器5存储10的指数的结果
         */
        huint1824_power_with_external_state(state,&state->state[5],&state->state[6],&state->state[7]);

        /*
         * 比较大小
         */
        switch(huint1824_compare(&state->state[5],src))
        {
        case 1:
        {
            is_ok=true;
...
size_t huint1824_dec_number(huint1824_state_t * state,const huint1824_t *src,size_t index)
{
     ...
     */
    huint1824_load_uint32(&state->state[6],10);
    huint1824_load_uint32(&state->state[7],index+1);
    /*
     * 寄存器5存储10的指数的结果
     */
    huint1824_power_with_external_state(state,&state->state[5],&state->state[6],&state->state[7]);
     ...
     * 第一次除法
     */
    huint1824_copy(&state->state[7],&state->state[5]);
    huint1824_div_with_external_state(state,&state->state[4],&state->state[5],src,&state->state[7]);

    /*
     * 寄存器6存储10，寄存器7存储10的指数
     */
    huint1824_load_uint32(&state->state[7],index);
    huint1824_power_with_external_state(state,&state->state[5],&state->state[6],&state->state[7]);

    /*
     * 第二次除法
     */
    huint1824_copy(&state->state[6],&state->state[4]);
    huint1824_copy(&state->state[7],&state->state[5]);
    huint1824_div_with_external_state(state,&state->state[4],&state->state[5],&state->state[6],&state->state[7]);

    uint32_t result=0;
    huint1824_store_uint32(&state->state[5],&result);
    dec_number=result;
