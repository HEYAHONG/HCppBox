...
size_t huint5792_clz(const huint5792_t *dst)
{
    if(dst!=NULL)
    {
        for(size_t i=0; i < HUINT5792_BITS_COUNT; i++)
        {
...
void huint5792_mul(huint5792_t *state,huint5792_t *dst,const huint5792_t *src1,const huint5792_t *src2)
{
    ...
    huint5792_load_uint32(dst,0);
    size_t clz=huint5792_clz(src2);
    size_t ctz=huint5792_ctz(src2);
    size_t last_index=ctz;
    huint5792_left_shift(state,src1,last_index);
    for(size_t i=ctz; i < (HUINT5792_BITS_COUNT-clz); i++)
    {
        if(huint5792_bit(src2,i))
        {
            //当前位是1, src1左移后累加至结果
            huint5792_left_shift(state,state,i-last_index);
            last_index=i;
...
void huint5792_div(huint5792_t *state,huint5792_t *state1,huint5792_t *state2,huint5792_t *dst,const huint5792_t *src1,const huint5792_t *src2)
{
    ...
    {
        huint5792_right_shift(state1,state1,i-last_index);
        last_index=i;
        if(huint5792_compare(state,state1) >= 0)
        {
            //被除数大于左移后的除数，直接相减并将相应位置1
            huint5792_sub(state2,state,state1);
