...
size_t huint5856_clz(const huint5856_t *dst)
{
    if(dst!=NULL)
    {
        for(size_t i=0; i < HUINT5856_BITS_COUNT; i++)
        {
...
void huint5856_mul(huint5856_t *state,huint5856_t *dst,const huint5856_t *src1,const huint5856_t *src2)
{
    ...
    huint5856_load_uint32(dst,0);
    size_t clz=huint5856_clz(src2);
    size_t ctz=huint5856_ctz(src2);
    size_t last_index=ctz;
    huint5856_left_shift(state,src1,last_index);
    for(size_t i=ctz; i < (HUINT5856_BITS_COUNT-clz); i++)
    {
        if(huint5856_bit(src2,i))
        {
            //当前位是1, src1左移后累加至结果
            huint5856_left_shift(state,state,i-last_index);
            last_index=i;
...
void huint5856_div(huint5856_t *state,huint5856_t *state1,huint5856_t *state2,huint5856_t *dst,const huint5856_t *src1,const huint5856_t *src2)
{
    ...
    {
        huint5856_right_shift(state1,state1,i-last_index);
        last_index=i;
        if(huint5856_compare(state,state1) >= 0)
        {
            //被除数大于左移后的除数，直接相减并将相应位置1
            huint5856_sub(state2,state,state1);
