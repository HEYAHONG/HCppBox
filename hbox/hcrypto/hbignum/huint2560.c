...
size_t huint2560_clz(const huint2560_t *dst)
{
    if(dst!=NULL)
    {
        for(size_t i=0; i < HUINT2560_BITS_COUNT; i++)
        {
...
void huint2560_mul(huint2560_t *state,huint2560_t *dst,const huint2560_t *src1,const huint2560_t *src2)
{
    ...
    huint2560_load_uint32(dst,0);
    size_t clz=huint2560_clz(src2);
    size_t ctz=huint2560_ctz(src2);
    size_t last_index=ctz;
    huint2560_left_shift(state,src1,last_index);
    for(size_t i=ctz; i < (HUINT2560_BITS_COUNT-clz); i++)
    {
        if(huint2560_bit(src2,i))
        {
            //当前位是1, src1左移后累加至结果
            huint2560_left_shift(state,state,i-last_index);
            last_index=i;
...
void huint2560_div(huint2560_t *state,huint2560_t *state1,huint2560_t *state2,huint2560_t *dst,const huint2560_t *src1,const huint2560_t *src2)
{
    ...
    {
        huint2560_right_shift(state1,state1,i-last_index);
        last_index=i;
        if(huint2560_compare(state,state1) >= 0)
        {
            //被除数大于左移后的除数，直接相减并将相应位置1
            huint2560_sub(state2,state,state1);
