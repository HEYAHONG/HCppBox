...
void huint160_load_be(huint160_t *num,const uint8_t *bytes,size_t bytes_count)
{
    ...
    {
        //丢弃高位
        size_t bytes_overflow=bytes_count-sizeof(huint160_t);
...
static size_t huint160_left_shift_internal(huint160_t *dst,const huint160_t *src,size_t bits)
{
    if(dst==NULL || src==NULL)
    {
        return 0;
...
void huint160_left_shift(huint160_t *dst,const huint160_t *src,size_t bits)
{
    ...
    huint160_copy(dst,src);

    while(bits > 0)
    {
        bits-=huint160_left_shift_internal(dst,dst,bits);
    }
}

static size_t huint160_right_shift_internal(huint160_t *dst,const huint160_t *src,size_t bits)
{
    if(dst==NULL || src==NULL)
    {
        return 0;
...
void huint160_right_shift(huint160_t *dst,const huint160_t *src,size_t bits)
{
    ...
    while(bits > 0)
    {
        bits-=huint160_right_shift_internal(dst,dst,bits);
    }
}
...
size_t huint160_clz(const huint160_t *dst)
{
    if(dst!=NULL)
    {
        ...
        for(size_t i=index_start; i < HUINT160_BITS_COUNT; i++)
        {
            if(huint160_bit(dst,HUINT160_BITS_COUNT-1-i))
            {
                return i;
            }
        }
...
bool huint160_t_is_zero(const huint160_t * src)
{
    bool ret=false;
    if(src==NULL)
    {
...
bool huint160_t_is_one(const huint160_t * src)
{
    bool ret=false;
    if(src==NULL)
    {
...
void huint160_sub(huint160_t *dst,const huint160_t *src1,const huint160_t *src2)
{
    ...
    //求补码
    huint160_complement(dst,src2);

    //对补码进行加
    huint160_add(dst,dst,src1);
}
...
void huint160_div(huint160_t *state,huint160_t *state1,huint160_t *state2,huint160_t *dst,const huint160_t *src1,const huint160_t *src2)
{
    ...
    if(huint160_compare(src2,src1) > 0)
    {
        //除数大于被除数
        huint160_copy(state,src1);
        huint160_load_uint32(dst,0);
        return;
    }

    size_t clz1=huint160_clz(src1);
    size_t clz2=huint160_clz(src2);
    if(clz1 > clz2)
    {
        //除数大于被除数
        huint160_copy(state,src1);
    ...
    huint160_copy(state,src1);
    size_t last_index=0;
    huint160_left_shift(state1,src2,(clz2-clz1)-last_index);
    for(size_t i=0; i<= clz2-clz1; i++)
    {
        huint160_right_shift_internal(state1,state1,i-last_index);
        last_index=i;
        if(huint160_compare(state,state1) >= 0)
        {
            //被除数大于左移后的除数，直接相减并将相应位置1
            {
                //求补码
                huint160_complement(state2,state1);
                //对补码进行加
                huint160_add(state,state,state2);
            }
            huint160_bit_set(dst,(clz2-clz1)-i);
        }
    }
...
void huint160_div_with_stack(huint160_t *mod,huint160_t *dst,const huint160_t *src1,const huint160_t *src2)
{
    ...
    huint160_div(&state[0],&state[1],&state[2],dst,src1,src2);
    if(mod!=NULL)
    {
        huint160_copy(mod,&state[0]);
    }
}
...
void huint160_power(huint160_t *state,huint160_t *state1,huint160_t *state2,huint160_t *dst,const huint160_t *src1,const huint160_t *src2)
{
    ...
    huint160_load_uint32(state,0);
    huint160_load_uint32(dst,1);
    if(huint160_compare(src1,state)==0 )
    {
        //底数为0
        huint160_load_uint32(dst,0);
        return;
    ...
    size_t clz2=huint160_clz(src2);
    huint160_copy(state,src1);
    for(size_t i=0; i<(HUINT160_BITS_COUNT - clz2); i++)
    {
        if(huint160_bit(src2,i))
        {
            //,结果应当乘上当前state的值(将幂函数的指数按照2进制进行拆分成乘法表达式)
            huint160_mul(state1,state2,dst,state);
            huint160_copy(dst,state2);
        }
        //计算state的平方（state的平方相当于state的指数乘2，正好对应src2下一个二进制位）
        huint160_mul(state1,state2,state,state);
        huint160_copy(state,state2);
    }
...
void huint160_root(huint160_t *state,huint160_t *state1,huint160_t *state2,huint160_t *state3,huint160_t *state4,huint160_t *dst,const huint160_t *src,size_t index)
{
    ...
    huint160_load_uint32(dst,0);
    huint160_load_uint64(state4,index);

    size_t dst_max_bit=(HUINT160_BITS_COUNT-huint160_clz(src)+index-1)/index;

    if(dst_max_bit*index > HUINT160_BITS_COUNT)
    {
        dst_max_bit--;
    }
    ...
    {
        huint160_bit_set(dst,dst_max_bit-i);
        huint160_power(state,state1,state2,state3,dst,state4);
        int compare_ret=huint160_compare(state3,src);
        if(compare_ret == 0)
        {
            break;
        }
        if(compare_ret > 0)
        {
            huint160_bit_clear(dst,dst_max_bit-i);
        }
    }

}

void huint160_root_with_stack(huint160_t *dst,const huint160_t *src,size_t index)
{
    huint160_t state[5];
    huint160_root(&state[0],&state[1],&state[2],&state[3],&state[4],dst,src,index);
}

void huint160_root_with_external_state(huint160_state_t * state,huint160_t *dst,const huint160_t *src,size_t index)
{
    if(state==NULL)
    {
        return;
    }
    huint160_root(&state->state[0],&state->state[1],&state->state[2],&state->state[3],&state->state[4],dst,src,index);
}
...
void huint160_power_mod(huint160_t *state,huint160_t *state1,huint160_t *state2,huint160_t *state3,huint160_t *dst,const huint160_t *src1,const huint160_t *src2,const huint160_t *src3)
{
    ...
    for(size_t i=0; i<(HUINT160_BITS_COUNT - clz2); i++)
    {
        if(huint160_bit(src2,i))
        {
            ...
            huint160_div(dst,state1,state2,state3,dst,src3);
        }
        //计算state的平方（state的平方相当于state的指数乘2，正好对应src2下一个二进制位）
        huint160_mul(state1,state2,state,state);
        huint160_copy(state,state2);
        ...
        huint160_div(state,state1,state2,state3,state,src3);
    }
...
size_t huint160_dec_number_count(huint160_state_t * state,const huint160_t *src)
{
    ...
    while(!is_ok)
    {
         ...
         */
        huint160_load_uint32(&state->state[6],10);
        huint160_load_uint32(&state->state[7],dec_number_count);
        /*
         * 寄存器5存储10的指数的结果
         */
        huint160_power_with_external_state(state,&state->state[5],&state->state[6],&state->state[7]);

        /*
         * 比较大小
         */
        switch(huint160_compare(&state->state[5],src))
        {
        case 1:
        {
            is_ok=true;
...
size_t huint160_dec_number(huint160_state_t * state,const huint160_t *src,size_t index)
{
    size_t dec_number=0;
    if(state==NULL || src==NULL)
    {
     ...
     */
    huint160_load_uint32(&state->state[6],10);
    huint160_load_uint32(&state->state[7],index+1);
    /*
     * 寄存器5存储10的指数的结果
     */
    huint160_power_with_external_state(state,&state->state[5],&state->state[6],&state->state[7]);
     ...
     * 第一次除法
     */
    huint160_copy(&state->state[7],&state->state[5]);
    huint160_div_with_external_state(state,&state->state[4],&state->state[5],src,&state->state[7]);

    /*
     * 寄存器6存储10，寄存器7存储10的指数
     */
    huint160_load_uint32(&state->state[7],index);
    huint160_power_with_external_state(state,&state->state[5],&state->state[6],&state->state[7]);

    /*
     * 第二次除法
     */
    huint160_copy(&state->state[6],&state->state[4]);
    huint160_copy(&state->state[7],&state->state[5]);
    huint160_div_with_external_state(state,&state->state[4],&state->state[5],&state->state[6],&state->state[7]);

    uint32_t result=0;
    huint160_store_uint32(&state->state[5],&result);
    dec_number=result;
